(function(){let e=document.createElement(`link`).relList;if(e&&e.supports&&e.supports(`modulepreload`))return;for(let e of document.querySelectorAll(`link[rel="modulepreload"]`))n(e);new MutationObserver(e=>{for(let t of e)if(t.type===`childList`)for(let e of t.addedNodes)e.tagName===`LINK`&&e.rel===`modulepreload`&&n(e)}).observe(document,{childList:!0,subtree:!0});function t(e){let t={};return e.integrity&&(t.integrity=e.integrity),e.referrerPolicy&&(t.referrerPolicy=e.referrerPolicy),e.crossOrigin===`use-credentials`?t.credentials=`include`:e.crossOrigin===`anonymous`?t.credentials=`omit`:t.credentials=`same-origin`,t}function n(e){if(e.ep)return;e.ep=!0;let n=t(e);fetch(e.href,n)}})();var e=`lightspeed-go-command-v2`,t=`http://127.0.0.1:8765`,n=[`Achilles`,`Neo`,`Architect`,`TheConstruct`,`Morpheus`,`Oracle`,`Smith`,`Merovingian`,`Trinity`],r=(e,t)=>e.replace(/\s+/g,` `).trim().slice(0,t),i=e=>{let t=e.toLowerCase();return/\b(ui|site|web|design|visual|layout|canva|accessibility)\b/.test(t)?`Trinity`:/\b(git|github|code|build|commit|branch|deploy|schema|api|runtime)\b/.test(t)?`Smith`:/\b(source|evidence|research|data|document|drive|sheet|workbook|citation)\b/.test(t)?`Oracle`:/\b(proof|claim|verify|conflict|confidence|audit)\b/.test(t)?`Morpheus`:/\b(simulate|simulation|model|gmat|trajectory|twin|physics)\b/.test(t)?`TheConstruct`:/\b(plan|mission|architecture|dependency|roadmap|system|project)\b/.test(t)?`Architect`:/\b(health|status|diagnostic|failure|error|monitor|storage|cleanup|archive)\b/.test(t)?`Merovingian`:/\b(coordinate|queue|handoff|agent|task|execute|run)\b/.test(t)?`Neo`:`Achilles`},a=t=>{let n=r(t.instruction,4e3);if(!n)throw TypeError(`instruction is required`);let a=new Date().toISOString(),o=Math.random().toString(36).slice(2,8).toUpperCase(),s=t.targetFloor??i(n);return{schema_version:e,command_id:`LSGO-${a.replace(/\D/g,``).slice(0,14)}-${o}`,created_utc:a,source:`LS GO`,title:r(t.title||n,160),instruction:n,target_floor:s,oversight_floor:`Achilles`,priority:t.priority??`normal`,execution_mode:t.executionMode??`review`,action_type:t.actionType??`cognigrex_workflow`,proof_required:!0,public_safe:!0}},o=async(e,t,n=3500)=>{let r=new AbortController,i=window.setTimeout(()=>r.abort(),n);try{let n=await fetch(e,{...t,signal:r.signal,cache:`no-store`});if(!n.ok){let e=`Desktop returned HTTP ${n.status}`;try{let t=await n.json();t.detail&&(e=t.detail)}catch{}throw Error(e)}return await n.json()}finally{window.clearTimeout(i)}},s=(e=t)=>o(`${e}/api/v1/status`,{method:`GET`},1e4),c=(e,n=t)=>o(`${n}/api/v1/ls-go/commands`,{method:`POST`,headers:{"Content-Type":`application/json`},body:JSON.stringify(e)},7e3),l=async(e=t)=>{let n=await o(`${e}/api/v1/tasks?limit=12`,{method:`GET`});return Array.isArray(n.tasks)?n.tasks:[]},ee=async(e=t)=>{let n=await o(`${e}/api/v1/projects`,{method:`GET`},15e3);return{projects:Array.isArray(n.projects)?n.projects:[],summary:n.summary||{},duplicateNames:Array.isArray(n.duplicate_names)?n.duplicate_names:[],cleanupSummary:n.cleanup_summary||{}}},u=async(e=t,n=50)=>{let r=await o(`${e}/api/v1/reviews?limit=${Math.max(1,Math.min(n,200))}`,{method:`GET`},1e4);return Array.isArray(r.reviews)?r.reviews:[]},d=async(e,n,i=``,a=t)=>o(`${a}/api/v1/reviews/${encodeURIComponent(e)}/decision`,{method:`POST`,headers:{"Content-Type":`application/json`},body:JSON.stringify({decision:n,note:r(i,1e3)})},7e3),te=async(e=t)=>{let n=await o(`${e}/api/v1/representation-graphs`,{method:`GET`},15e3);return Array.isArray(n.graphs)?n.graphs:[]},ne=async(e,n,i,a=[],s=``,c=``,l=t)=>o(`${l}/api/v1/representation-reviews/${encodeURIComponent(e)}/decision`,{method:`POST`,headers:{"Content-Type":`application/json`,"X-LightSpeed-Owner-Confirmation":c.slice(0,256)},body:JSON.stringify({decision:n,scope:i,edge_ids:a.slice(0,100),note:r(s,1e3)})},1e4),f=`lightspeed-go-pending-commands-v1`,p=()=>{try{let e=JSON.parse(localStorage.getItem(f)||`[]`);return Array.isArray(e)?e:[]}catch{return[]}},m=e=>{let t=[e,...p().filter(t=>t.command_id!==e.command_id)].slice(0,30);return localStorage.setItem(f,JSON.stringify(t)),t},re=e=>{let t=p().filter(t=>t.command_id!==e);return localStorage.setItem(f,JSON.stringify(t)),t},ie=e=>{let t=new Blob([JSON.stringify(e,null,2)],{type:`application/json`}),n=URL.createObjectURL(t),r=document.createElement(`a`);r.href=n,r.download=`${e.command_id}.json`,r.click(),URL.revokeObjectURL(n)},ae=`lightspeed-neo-exchange-v1`,oe=[`critical`,`high`,`normal`,`low`],se=[`queued`,`active`,`review`,`blocked`,`complete`],ce=[`icon`,`age_label`],h=e=>typeof e==`object`&&!!e&&!Array.isArray(e),g=(e,t,n)=>{if(typeof e!=`string`)return t;let r=e.replace(/\s+/g,` `).trim();return r?r.slice(0,n):t},_=(e,t,n)=>{let r=g(e,``,n);if(!r)throw TypeError(`queue record ${t} is required`);return r},v=(e,t,n)=>typeof e==`string`&&t.includes(e)?e:n,le=e=>h(e)?Object.fromEntries(ce.flatMap(t=>{let n=g(e[t],``,48);return n?[[t,n]]:[]})):{},ue=e=>{if(!h(e))throw TypeError(`queue record must be an object`);return{id:_(e.id,`id`,80),title:_(e.title,`title`,160),priority:v(e.priority,oe,`normal`),status:v(e.status,se,`queued`),source:g(e.source,`GO Gate`,48),target:g(e.target,`Neo`,48),created_utc:g(e.created_utc,``,32),extensions:le(e.extensions),notes:g(e.notes,``,240)}},de=e=>{if(!h(e))throw TypeError(`Neo exchange must be an object`);let t=Array.isArray(e.queue)?e.queue.map(ue):[];return{schema_version:ae,generated_at_utc:g(e.generated_at_utc,``,32),queue:t}},fe=e=>({total:e.queue.length,critical:e.queue.filter(e=>e.priority===`critical`).length,active:e.queue.filter(e=>e.status!==`complete`).length,complete:e.queue.filter(e=>e.status===`complete`).length}),y=e=>e.replace(/[&<>"']/g,e=>({"&":`&amp;`,"<":`&lt;`,">":`&gt;`,'"':`&quot;`,"'":`&#039;`})[e]??e),pe=async e=>{try{return de(await e())}catch{return de({})}},me=e=>e.status===`blocked`?`blocked`:e.status===`complete`?`pass`:e.priority===`critical`||e.priority===`high`?`warn`:`ready`,he=e=>{let t=fe(e),n=e.queue.length?e.queue.map(e=>`
            <li class="status-row ${me(e)}">
              <div>
                <strong>${y(e.title)}</strong>
                <span>${y(e.source)} to ${y(e.target)} · ${y(e.id)}</span>
                ${e.notes?`<small>${y(e.notes)}</small>`:``}
              </div>
              <em>${y(e.status)}</em>
            </li>
          `).join(``):`
  ...
  `},b=e=>String(e??``).replace(/&/g,`&amp;`).replace(/</g,`&lt;`).replace(/>/g,`&gt;`).replace(/"/g,`&quot;`).replace(/'/g,`&#39;`),x=e=>e?`${e.slice(0,12)}…${e.slice(-8)}`:`not available`,ge=e=>e.path_exposed===!1?String(e.label||`private local evidence`):[e.repository,e.commit_sha?`commit ${String(e.commit_sha).slice(0,12)}`:null,e.path,e.drive_file_id?`Drive ${e.drive_file_id}`:null,e.sheet_name,e.stable_key||e.content_key,e.missing_state].filter(Boolean).map(b).join(` · `)||b(e.locator_type||`logical`),S=e=>b(JSON.stringify(e,null,2)),_e=e=>{let t=e.representations.filter(e=>e.state===`active`).length,n=e.missing.length;return!t&&n?`unable to determine`:n>t?`smaller bowl`:t>n?`larger bowl`:`unchanged bowl`},ve=e=>{let t=e.review,n=t?.review_stage||`identity`,r=e.edges.map(e=>e.edge_id).join(`|`),i=t?`
    <div class="graph-actions" data-review-stage="${b(n)}">
      <strong>${n===`identity`?`Review identity first`:`Review ${e.edges.length} bounded edges`}</strong>
      <div class="task-actions">
        <button data-representation-review="${b(t.review_id)}" data-scope="${n}" data-edge-ids="${b(r)}" data-decision="approve">Approve</button>
        <button data-representation-review="${b(t.review_id)}" data-scope="${n}" data-edge-ids="${b(r)}" data-decision="provisional_approve">Provisional</button>
        <button data-representation-review="${b(t.review_id)}" data-scope="${n}" data-edge-ids="${b(r)}" data-decision="hold">Hold</button>
        <button data-representation-review="${b(t.review_id)}" data-scope="${n}" data-edge-ids="${b(r)}" data-decision="request_evidence">Request evidence</button>
        <button data-representation-review="${b(t.review_id)}" data-scope="${n}" data-edge-ids="${b(r)}" data-decision="reject">Reject</button>
        <button data-representation-review="${b(t.review_id)}" data-scope="${n}" data-edge-ids="${b(r)}" data-decision="supersede">Supersede</button>
      </div>
      <small>${b(t.state)} · graph ${x(t.graph_sha256)}</small>
    </div>`:`<p class="muted">Review packet has not been staged.</p>`,a=e.representations.map(e=>`
    <tr>
      <td><strong>${b(e.representation_type)}</strong><small>${b(e.representation_id)}</small></td>
      <td>${ge(e.locator)}</td>
      <td>${b(e.source_authority)}</td>
      <td>${x(e.content_sha256)}</td>
      <td>${b(e.confidence_class)} (${Math.round(e.confidence_numeric*100)}%)</td>
      <td><span class="state-chip" data-state="${b(e.state)}">${b(e.state)}</span></td>
      <td>${b(e.claim_boundary)}</td>
    </tr>`).join(``),o=e.edges.map(e=>`
    <tr>
      <td>${b(e.from_representation_id)}</td>
      <td><strong>${b(e.relation)}</strong></td>
      <td>${b(e.to_representation_id)}</td>
      <td>${b(e.evidence_bundle_id||`not required`)}</td>
      <td>${b(e.review_state)}</td>
      <td>${b(e.claim_boundary)}</td>
    </tr>`).join(``),s=e.missing.length?e.missing.map(e=>`
      <article class="missing-card">
        <strong>${b(e.type)} · ${b(e.missing_state)}</strong>
        <span><b>Why:</b> ${b(e.reason)}</span>
        <span><b>Next:</b> ${b(e.next_evidence_action)}</span>
        <small>Last search: ${b(e.last_search||`not recorded`)} · Floor: ${b(e.assigned_floor)} · Effect: ${b(e.dependency_effect)}</small>
      </article>`).join(``):`<p class="muted">No required representation is currently missing.</p>`,c=e.conflicts.length?e.conflicts.map(e=>`<article class="missing-card conflict"><strong>${b(e.edge_id)}</strong><span>${b(e.claim_boundary)}</span></article>`).join(``):`<p class="muted">No representation conflict is recorded.</p>`,l=e.horizons.length?e.horizons.map(e=>`
      <article class="horizon-card">
        <div><strong>${b(e.name)}</strong><span>${b(e.state)} · ${b(e.horizon_type)}</span></div>
        <p>${b(e.objective)}</p>
        <details><summary>Assumptions and constraints</summary><pre>${S({assumptions:e.assumptions,constraints:e.constraints})}</pre></details>
        <small>Input ${x(e.input_set_sha256)}</small>
      </article>`).join(``):`<p class="muted">No horizon is assigned.</p>`,ee=e.representations.find(e=>e.representation_type===`recommendation`)?.locator.next_highest_value_question,u=(e.linked_objects||[]).length?(e.linked_objects||[]).map(t=>{let n=(e.linked_identifiers||[]).filter(e=>e.object_id===t.object_id).map(e=>`${e.namespace}: ${e.identifier_value}`).join(` | `);return`<article class="missing-card">
        <strong>${b(t.display_name)} · ${b(t.state)}</strong>
        <span><b>Object:</b> ${b(t.object_id)}</span>
        <span><b>Identifiers:</b> ${b(n||`none recorded`)}</span>
      </article>`}).join(``):`<p class="muted">No linked object identity is included.</p>`,d=(e.evidence_bundles||[]).length?(e.evidence_bundles||[]).map(e=>`
      <article class="horizon-card">
        <div><strong>${b(e.title)}</strong><span>${b(e.state)}</span></div>
        <p>${b(e.claim_boundary)}</p>
        ...
        <details><summary>Source weight summary</summary><pre>${S(e.source_weight_summary)}</pre></details>
      </article>`).join(``):`<p class="muted">No evidence bundle is linked.</p>`;return`
    <article class="panel graph-panel" data-object-id="${b(e.object.object_id)}">
      <div class="panel-head">
        <div>
          <p class="eyebrow">${b(e.object.object_type)}</p>
          <h2>${b(e.object.display_name)}</h2>
          <p>${b(e.object.description)}</p>
        </div>
        <span class="badge">${b(e.canonical_state)}</span>
      </div>
      <div class="graph-summary">
        <div><span>Object ID</span><strong>${b(e.object.object_id)}</strong></div>
        <div><span>Authority</span><strong>${b(e.object.authority)}</strong></div>
        <div><span>Identity</span><strong>${b(e.object.identity_confidence_class)} · ${Math.round(e.object.identity_confidence_numeric*100)}%</strong></div>
        <div><span>Current horizon</span><strong>${b(e.horizons[0]?.name||`not assigned`)}</strong></div>
        <div><span>Judgment</span><strong>${_e(e)}</strong></div>
      </div>
      <details open><summary>Identifiers (${e.identifiers.length})</summary><div class="identifier-list">${e.identifiers.map(e=>`<span><strong>${b(e.namespace)}</strong>${b(e.identifier_value)} · ${b(e.authority)}</span>`).join(``)}</div></details>
      <details open><summary>Linked identities (${(e.linked_objects||[]).length})</summary><div class="graph-grid">${u}</div></details>
...
`;var w=e=>{let t=document.getElementById(e);if(!t)throw Error(`Missing #${e}`);return t},T=w(`instruction`),E=w(`target-floor`),Ce=w(`priority`),we=w(`execution-mode`),Te=w(`route-preview`),D=w(`command-result`),O=null,k=[],A=[],j=()=>{let e=i(T.value||`governance`);E.value=e,Te.innerHTML=`<strong>Achilles route:</strong> ${e} is primary. Neo coordinates and proof returns to this gate.`};T.addEventListener(`input`,j),j();var M=()=>a({instruction:T.value,targetFloor:E.value,priority:Ce.value,executionMode:we.value}),N=(e,t)=>{D.dataset.tone=e,D.textContent=t},P=()=>{let e=p();w(`pending-count`).textContent=String(e.length);let t=w(`pending-commands`);if(!e.length){t.innerHTML=`<p class="muted">No locally saved commands.</p>`;return}t.innerHTML=e.map(e=>`<article class="task-card"><div><strong>${y(e.title)}</strong><span>${y(e.target_floor)} · ${y(e.priority)} · ${y(e.execution_mode)}</span><small>${y(e.command_id)}</small></div><div class="task-actions"><button data-send="${y(e.command_id)}">Send</button><button data-download="${y(e.command_id)}">Download</button></div></article>`).join(``),t.querySelectorAll(`[data-send]`).forEach(t=>t.addEventListener(`click`,async()=>{let n=e.find(e=>e.command_id===t.dataset.send);if(n)try{let e=await c(n);re(n.command_id),P(),N(`good`,`Desktop accepted ${e.command_id||n.command_id}. Task ${e.task_id??`created`}.`),await R()}catch(e){N(`bad`,e instanceof Error?e.message:`Desktop command failed.`)}})),t.querySelectorAll(`[data-download]`).forEach(t=>t.addEventListener(`click`,()=>{let n=e.find(e=>e.command_id===t.dataset.download);n&&ie(n)}))},Ee=e=>{let t=Number(e||0);return t<1024?`${t} B`:t<1024**2?`${(t/1024).toFixed(1)} KB`:t<1024**3?`${(t/1024**2).toFixed(1)} MB`:`${(t/1024**3).toFixed(1)} GB`},F=e=>{let t=w(`desktop-projects`);if(w(`project-count`).textContent=String(e.length),!e.length){t.innerHTML=`<p class="muted">No project folders were found in the configured roots.</p>`;return}t.innerHTML=e.slice(0,30).map(e=>`<article class="task-card"><div><strong>${y(e.name)}</strong><span>${y(e.condition||`unknown`)} · ${y(e.authority||`reference`)} · ${e.file_count||0} files</span><small>${Ee(e.size_bytes)}${e.scan_truncated?` · bounded scan`:``}</small></div></article>`).join(``)},I=e=>{k=e;let t=w(`desktop-reviews`);if(!e.length){t.innerHTML=`<p class="muted">No project receipts are awaiting review.</p>`;return}t.innerHTML=e.slice(0,30).map(e=>{let t=e.state||`pending_review`,n=t===`pending_review`?`<div class="task-actions"><button data-review="${y(e.review_id)}" data-decision="approve">Approve</button><button data-review="${y(e.review_id)}" data-decision="hold">Hold</button><button data-review="${y(e.review_id)}" data-decision="reject">Reject</button></div>`:``;return`<article class="task-card"><div><strong>${y(e.title||`Project receipt`)}</strong><span>${y(t)} · ${y(e.event_type||`receipt`)}</span><small>${y(e.summary||e.review_id)}</small></div>${n}</article>`}).join(``),t.querySelectorAll(`[data-review]`).forEach(e=>e.addEventListener(`click`,async()=>{let t=e.dataset.review||``,n=e.dataset.decision,r=k.find(e=>e.review_id===t);if(!r||!t)return;let i=window.prompt(`${n.toUpperCase()}: ${r.title||t}\nOptional decision note:`,``)??``;try{await d(t,n,i),N(`good`,`${t} marked ${n}. Drive decision receipt written by Desktop.`),await R()}catch(e){N(`bad`,e instanceof Error?e.message:`Review decision failed.`)}}))},L=e=>{A=e;let t=w(`representation-graphs`);t.innerHTML=ye(e),t.querySelectorAll(`[data-representation-review]`).forEach(e=>{e.addEventListener(`click`,async()=>{let t=e.dataset.representationReview||``,n=e.dataset.decision,r=e.dataset.scope||`identity`,i=r===`edges`?(e.dataset.edgeIds||``).split(`|`).filter(Boolean).slice(0,100):[],a=A.find(e=>e.review?.review_id===t);if(!t||!a)return;let o=window.prompt(`${n.replace(/_/g,` `).toUpperCase()}: ${a.object.display_name}\n${r===`identity`?`Identity is reviewed before edges.`:`${i.length} bounded edges selected.`}\nOptional decision note:`,``)??``,s=window.prompt(`Enter the local owner-confirmation token. It is sent only to the loopback Desktop bridge and is not stored.`,``)??``;if(!s){N(`bad`,`Representation decision cancelled: owner confirmation is required.`);return}try{await ne(t,n,r,i,o,s),N(`good`,`${t} recorded ${n}; local staging remains noncanonical until Drive readback.`),await R()}catch(e){N(`bad`,e instanceof Error?e.message:`Representation review decision failed.`)}})})};w(`command-form`).addEventListener(`submit`,async e=>{e.preventDefault();try{O=M(),N(`warn`,`Sending ${O.command_id} to Desktop…`);let e=await c(O);re(O.command_id),P(),N(`good`,`Accepted by Desktop. Task ${e.task_id??`created`}; ${e.state||`queued for governed processing`}.`),await R()}catch(e){O&&m(O),P(),N(`bad`,`${e instanceof Error?e.message:`Desktop unavailable`} The command envelope was saved locally.`)}}),w(`save-command`).addEventListener(`click`,()=>{try{O=M(),m(O),P(),N(`good`,`${O.command_id} saved locally.`)}catch(e){N(`bad`,e instanceof Error?e.message:`Command could not be saved.`)}}),w(`copy-command`).addEventListener(`click`,async()=>{try{O=M(),await navigator.clipboard.writeText(JSON.stringify(O,null,2)),N(`good`,`${O.command_id} copied as JSON.`)}catch(e){N(`bad`,e instanceof Error?e.message:`Command could not be copied.`)}});var R=async()=>{let e=w(`desktop-pill`),t=w(`desktop-state`),n=w(`merovingian-state`),r=w(`desktop-pill-text`),i=w(`desktop-tasks`);e.dataset.state=`checking`,r.textContent=`checking local runtime`,t.textContent=`Checking`,n.textContent=`Checking`;try{let a=await s();e.dataset.state=a.ok?`online`:`degraded`,r.textContent=a.ok?`local runtime connected`:`runtime connected; health needs review`,t.textContent=`Online`,n.textContent=a.merovingian?.status===`pass`?`Healthy`:`Degraded`;try{let e=await l();i.innerHTML=e.length?e.map(e=>`<article class="task-card"><div><strong>${y(String(e.title||`Untitled task`))}</strong><span>${y(String(e.status||`unknown`))} · ${y(String(e.priority||`normal`))}</span><small>Task ${y(String(e.id||``))}</small></div></article>`).join(``):`<p class="muted">Desktop queue is clear.</p>`}catch{i.innerHTML=`<p class="muted">Desktop is online, but task listing is unavailable.</p>`}try{F((await ee()).projects)}catch{F([])}try{I(await u())}catch{I([])}try{L(await te())}catch{L([])}}catch{e.dataset.state=`offline`,r.textContent=`start LightSpeed Desktop and the local bridge`,t.textContent=`Offline`,n.textContent=`Offline`,w(`project-count`).textContent=`0`,i.innerHTML=`<p class="muted">Desktop is offline. Commands can still be saved, copied or downloaded.</p>`,w(`desktop-projects`).innerHTML=`<p class="muted">Project registry unavailable while Desktop is offline.</p>`,w(`desktop-reviews`).innerHTML=`<p class="muted">Review queue unavailable while Desktop is offline.</p>`,L([])}};w(`refresh-desktop`).addEventListener(`click`,()=>void R()),P(),R();var De=w(`neo-exchange`),Oe=new URL(`./data/neo_exchange.json`,document.baseURI).toString();pe(async()=>{let e=await fetch(Oe,{cache:`no-store`});if(!e.ok)throw Error(`Neo exchange returned HTTP ${e.status}`);return e.json()}).then(e=>{De.innerHTML=he(e)}),document.querySelectorAll(`.tab`).forEach(e=>e.addEventListener(`click`,()=>{document.querySelectorAll(`.tab`).forEach(e=>e.classList.remove(`active`)),document.querySelectorAll(`.view`).forEach(e=>e.classList.remove(`active`)),e.classList.add(`active`),w(`view-${e.dataset.view}`).classList.add(`active`)}));var ke=new URL(`./data/site_integration.json`,document.baseURI).toString(),Ae=async()=>{try{let e=await fetch(ke,{cache:`no-store`});return e.ok?await e.json():null}catch{return null}},je=()=>{if(document.getElementById(`lsgo-sites-side-edit-styles`))return;let e=document.createElement(`style`);e.id=`lsgo-sites-side-edit-styles`,e.textContent=`
  ...
  `,document.head.appendChild(e)},Me=async()=>{let e=document.querySelector(`.topbar > div:first-child`),t=document.querySelector(`#view-sources`);if(!e||!t)return!1;je();let n=(await Ae())?.authority_chain??[`Nathaniel Bouwer`,`Achilles / GO gate`,`agent floor`,`LightSpeed Desktop`,`Git and Drive receipts`];if(!document.getElementById(`site-context-strip`)){let t=document.createElement(`div`);t.id=`site-context-strip`,t.className=`site-context-strip`,t.innerHTML=`
  ...
  `,e.querySelectorAll(`[data-ls-agent]`).forEach(e=>{e.addEventListener(`click`,()=>Z(e.dataset.lsAgent,!0))}),e.querySelector(`[data-ls-home]`)?.addEventListener(`click`,()=>X(`command`)),e.querySelector(`[data-ls-focus]`)?.addEventListener(`click`,()=>{let e=!q.focusMode;J({focusMode:e}),document.body.dataset.lsFocus=e?`true`:`false`}),e},Re=()=>{let e=document.createElement(`section`);return e.className=`ls-os-status-strip`,e.setAttribute(`aria-label`,`LightSpeed operating state`),e.innerHTML=`
  ...
  `,e.querySelectorAll(`[data-ls-stage]`).forEach(e=>{e.addEventListener(`click`,()=>Q(e.dataset.lsStage))}),e},Be=()=>{let e=document.createElement(`div`);return e.className=`ls-os-palette`,e.id=`ls-os-palette`,e.hidden=!0,e.innerHTML=`
