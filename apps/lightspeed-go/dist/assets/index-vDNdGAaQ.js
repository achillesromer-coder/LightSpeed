...
`,Oe=e=>{let t=e.preview,n=`<p class="muted">Metadata only. Binary or non-UTF-8 content is not transferred into LS GO.</p>`;return t.state===`empty`?n=`<p class="muted">The file is empty.</p>`:t.state===`available`&&(n=`<pre>${b(t.text||``)}</pre>${t.truncated?`<small class="muted">Preview truncated at the governed byte limit.</small>`:``}`),`
  ...
  `},ke=(e,t)=>{e.querySelectorAll(`[data-project-files]`).forEach(e=>{e.addEventListener(`click`,()=>{let n=e.dataset.projectFiles||``;n&&t(n,e)})})},Ae=(e,t)=>{e.querySelectorAll(`[data-project-file][data-project-id]`).forEach(e=>{e.addEventListener(`click`,()=>{let n=e.dataset.projectId||``,r=e.dataset.projectFile||``;n&&r&&t(n,r,e)})})},je=e=>{let t=Number(e||0);return t<1024?`${t} B`:t<1024**2?`${(t/1024).toFixed(1)} KB`:`${(t/1024**2).toFixed(1)} MB`},Me=e=>{let t=String(e.status||`unknown`).toLowerCase();return[`complete`,`completed`,`pass`,`passed`].includes(t)?`good`:[`blocked`,`failed`,`error`].includes(t)?`bad`:`warn`},Ne=(e,t)=>{let n=e.summary,r=`<p class="result-receipt-boundary">${b(e.boundary)}</p>`,i=t?`Exact receipt content requires the local owner-confirmation token.`:`Content inspection is held because the local owner-confirmation token is not configured.`;if(!e.results.length)return`<p class="muted">${e.state===`restricted`?`No eligible fixed receipts are visible; invalid receipt objects remain withheld.`:`No fixed local result receipts have been written yet.`}</p><p class="result-receipt-auth">${i}</p>${r}`;let a=e.results.map(e=>{let n=e.action_type||`untyped`,r=e.target_floor||`floor unknown`,i=e.completed_utc||e.created_utc||e.modified_utc,a=[e.task_id==null?null:`Task ${e.task_id}`,e.job_id==null?null:`Job ${e.job_id}`,e.command_id||null].filter(Boolean).join(` · `),o=t?``:` disabled aria-disabled="true"`;return`
    ...
    ${r}
  `},Pe=e=>{let t=JSON.stringify(e.result,null,2);return`
    <section class="result-receipt-preview">
      <div class="result-receipt-preview-head">
        <strong>${b(e.identity.result_id)}</strong>
        <small>${je(e.identity.size_bytes)} · SHA-256 ${b(e.identity.sha256)}</small>
      </div>
      <pre>${b(t)}</pre>
  ...
  `},C=e=>`
  <p class="result" data-tone="bad">${b(e)}</p>
`,Fe=(e,t)=>{e.querySelectorAll(`[data-result-receipt]`).forEach(e=>{e.addEventListener(`click`,()=>{let n=e.dataset.resultReceipt||``;n&&!e.disabled&&t(n,e)})})},w=e=>String(e??``).replace(/&/g,`&amp;`).replace(/</g,`&lt;`).replace(/>/g,`&gt;`).replace(/"/g,`&quot;`).replace(/'/g,`&#39;`),T=e=>e?`${e.slice(0,12)}…${e.slice(-8)}`:`not available`,Ie=e=>e.path_exposed===!1?String(e.label||`private local evidence`):[e.repository,e.commit_sha?`commit ${String(e.commit_sha).slice(0,12)}`:null,e.path,e.drive_file_id?`Drive ${e.drive_file_id}`:null,e.sheet_name,e.stable_key||e.content_key,e.missing_state].filter(Boolean).map(w).join(` · `)||w(e.locator_type||`logical`),Le=e=>w(JSON.stringify(e,null,2)),Re=e=>{let t=e.representations.filter(e=>e.state===`active`).length,n=e.missing.length;return!t&&n?`unable to determine`:n>t?`smaller bowl`:t>n?`larger bowl`:`unchanged bowl`},ze=e=>{let t=e.review,n=t?.review_stage||`identity`,r=e.edges.map(e=>e.edge_id).join(`|`),i=t?`
    ...
    <tr>
      ...
      <article class="horizon-card">
        ...
        <p>${w(e.objective)}</p>
        <details><summary>Assumptions and constraints</summary><pre>${Le({assumptions:e.assumptions,constraints:e.constraints})}</pre></details>
      ...
      <article class="horizon-card">
        ...
        <details><summary>Source weight summary</summary><pre>${Le(e.source_weight_summary)}</pre></details>
...
`;var E=e=>{let t=document.getElementById(e);if(!t)throw Error(`Missing #${e}`);return t},D=E(`instruction`),Ke=E(`target-floor`),qe=E(`priority`),Je=E(`execution-mode`),Ye=E(`route-preview`),Xe=E(`command-result`),O=null,k=null,Ze=[],A=[],j=``,M=``,N=`NCNB`,P=(e,t)=>{let n=E(`owner-auth-result`);n.dataset.tone=e,n.textContent=t},Qe=e=>{let t=E(`owner-login-form`),n=E(`owner-change-form`),r=E(`owner-logout`);if(N=e.credential?.username||N,j=e.session_token||``,M=e.password_change_token||``,E(`owner-password`).value=``,e.authenticated&&j){n.hidden=!0,r.hidden=!1,E(`owner-auth-state`).textContent=`${N} signed in`,P(`good`,`Owner session active until ${e.expires_utc||`page close`}.`);return}r.hidden=!0,n.hidden=!e.change_required,t.hidden=!1,E(`owner-auth-state`).textContent=e.change_required?`Password change required`:`Sign-in required`,P(`warn`,e.change_required?`The bootstrap password was verified. Enter it again with a new password to complete first login.`:`Owner sign-in is required for unredacted files, receipts, and decisions.`)},F=()=>j||(P(`warn`,`Sign in as NCNB before performing this owner-gated action.`),``);E(`owner-login-form`).addEventListener(`submit`,async e=>{e.preventDefault();let t=E(`owner-username`).value.trim(),n=E(`owner-password`);try{Qe(await l(t,n.value))}catch(e){n.value=``,P(`bad`,e instanceof Error?e.message:`Owner sign-in failed.`)}}),E(`owner-change-form`).addEventListener(`submit`,async e=>{e.preventDefault();let t=E(`owner-current-password`),n=E(`owner-new-password`),r=E(`owner-confirm-password`);if(n.value!==r.value){P(`bad`,`The new passwords do not match.`);return}let i=M||j;if(!i){P(`bad`,`Sign in before changing the password.`);return}try{let e=await u(N,t.value,n.value,i,!!M);t.value=``,n.value=``,r.value=``,Qe(e)}catch(e){t.value=``,n.value=``,r.value=``,P(`bad`,e instanceof Error?e.message:`Password change failed.`)}}),E(`owner-logout`).addEventListener(`click`,async()=>{let e=j;if(j=``,M=``,e)try{await d(e)}catch{}E(`owner-logout`).hidden=!0,E(`owner-auth-state`).textContent=`Signed out`,P(`warn`,`Owner session cleared from this page.`)});var I=()=>{let e=i(D.value||`governance`);Ke.value=e;let t=k?.canonical_gate_id;Ye.innerHTML=`<strong>Achilles route:</strong> ${e} is primary. Neo coordinates and proof returns to this gate.${t?` <small>Authority: ${b(t)}</small>`:` <small>Waiting for the Desktop authority contract.</small>`}`};D.addEventListener(`input`,I),I();var L=()=>a({instruction:D.value,targetFloor:Ke.value,priority:qe.value,executionMode:Je.value,authorityContract:k}),R=(e,t)=>{Xe.dataset.tone=e,Xe.textContent=t},z=()=>{let e=_();E(`pending-count`).textContent=String(e.length);let t=E(`pending-commands`);if(!e.length){t.innerHTML=`<p class="muted">No locally saved commands.</p>`;return}t.innerHTML=e.map(e=>`<article class="task-card"><div><strong>${b(e.title)}</strong><span>${b(e.target_floor)} · ${b(e.priority)} · ${b(e.execution_mode)}</span><small>${b(e.command_id)}</small></div><div class="task-actions"><button data-send="${b(e.command_id)}">Send</button><button data-download="${b(e.command_id)}">Download</button></div></article>`).join(``),t.querySelectorAll(`[data-send]`).forEach(t=>t.addEventListener(`click`,async()=>{let n=e.find(e=>e.command_id===t.dataset.send);if(n)try{let e=await f(n);de(n.command_id),z(),R(`good`,`Desktop accepted ${e.command_id||n.command_id}. Task ${e.task_id??`created`}.`),await B()}catch(e){R(`bad`,e instanceof Error?e.message:`Desktop command failed.`)}})),t.querySelectorAll(`[data-download]`).forEach(t=>t.addEventListener(`click`,()=>{let n=e.find(e=>e.command_id===t.dataset.download);n&&fe(n)}))},$e=e=>{let t=E(`desktop-projects`);E(`project-count`).textContent=String(e.length),t.innerHTML=Ee(e),ke(t,async(e,t)=>{let n=t.closest(`[data-project-card]`)?.querySelector(`.project-files`);if(n){if(t.getAttribute(`aria-expanded`)===`true`){t.setAttribute(`aria-expanded`,`false`),t.textContent=`Files`,n.hidden=!0;return}t.disabled=!0,t.textContent=`Loading…`,n.hidden=!1,n.innerHTML=`<p class="muted">Reading bounded project metadata…</p>`;try{n.innerHTML=De(await ee(e)),Ae(n,async(e,t,r)=>{let i=n.querySelector(`.project-file-result`);if(!i)return;let a=F();if(!a){i.innerHTML=S(`File preview held: sign in through the NCNB owner gate first.`);return}r.disabled=!0,i.innerHTML=`<p class="muted">Opening read-only result…</p>`;try{i.innerHTML=Oe(await te(e,t,a))}catch(e){i.innerHTML=S(e instanceof Error?e.message:`Project file result is unavailable.`)}finally{r.disabled=!1}}),t.setAttribute(`aria-expanded`,`true`),t.textContent=`Hide files`}catch(e){n.innerHTML=S(e instanceof Error?e.message:`Project files are unavailable.`),t.textContent=`Retry files`}finally{t.disabled=!1}}})},et=(e,t)=>{let n=E(`desktop-results`),r=E(`result-auth-state`);r.textContent=t?`Owner gate configured`:`Content gate held`,n.innerHTML=Ne(e,t),Fe(n,async(e,r)=>{let i=n.querySelector(`.result-receipt-detail`);if(!i)return;if(!t){i.innerHTML=C(`Receipt content is held because owner confirmation is not configured on the local bridge.`);return}let a=F();if(!a){i.innerHTML=C(`Receipt inspection held: sign in through the NCNB owner gate first.`);return}r.disabled=!0,i.innerHTML=`<p class="muted">Opening owner-confirmed read-only receipt…</p>`;try{i.innerHTML=Pe(await ie(e,a))}catch(e){i.innerHTML=C(e instanceof Error?e.message:`Local result receipt is unavailable.`)}finally{r.disabled=!1}})},tt=e=>{Ze=e;let t=E(`desktop-reviews`);if(!e.length){t.innerHTML=`<p class="muted">No project receipts are awaiting review.</p>`;return}t.innerHTML=e.slice(0,30).map(e=>{let t=e.state||`pending_review`,n=t===`pending_review`?`<div class="task-actions"><button data-review="${b(e.review_id)}" data-decision="approve">Approve</button><button data-review="${b(e.review_id)}" data-decision="hold">Hold</button><button data-review="${b(e.review_id)}" data-decision="reject">Reject</button></div>`:``;return`<article class="task-card"><div><strong>${b(e.title||`Project receipt`)}</strong><span>${b(t)} · ${b(e.event_type||`receipt`)}</span><small>${b(e.summary||e.review_id)}</small></div>${n}</article>`}).join(``),t.querySelectorAll(`[data-review]`).forEach(e=>e.addEventListener(`click`,async()=>{let t=e.dataset.review||``,n=e.dataset.decision,r=Ze.find(e=>e.review_id===t);if(!r||!t)return;let i=window.prompt(`${n.toUpperCase()}: ${r.title||t}\nOptional decision note:`,``)??``,a=F();if(!a){R(`bad`,`Review decision held: sign in through the NCNB owner gate first.`);return}try{R(`good`,se(t,n,await oe(t,n,i,a))),await B()}catch(e){R(`bad`,e instanceof Error?e.message:`Review decision failed.`)}}))},nt=e=>{A=e;let t=E(`representation-graphs`);t.innerHTML=Be(e),t.querySelectorAll(`[data-representation-review]`).forEach(e=>{e.addEventListener(`click`,async()=>{let t=e.dataset.representationReview||``,n=e.dataset.decision,r=e.dataset.scope||`identity`,i=r===`edges`?(e.dataset.edgeIds||``).split(`|`).filter(Boolean).slice(0,100):[],a=A.find(e=>e.review?.review_id===t);if(!t||!a)return;let o=window.prompt(`${n.replace(/_/g,` `).toUpperCase()}: ${a.object.display_name}\n${r===`identity`?`Identity is reviewed before edges.`:`${i.length} bounded edges selected.`}\nOptional decision note:`,``)??``,s=F();if(!s){R(`bad`,`Representation decision held: sign in through the NCNB owner gate first.`);return}try{await le(t,n,r,i,o,s),R(`good`,`${t} recorded ${n}; local staging remains noncanonical until Drive readback.`),await B()}catch(e){R(`bad`,e instanceof Error?e.message:`Representation review decision failed.`)}})})};E(`command-form`).addEventListener(`submit`,async e=>{e.preventDefault(),O=null;try{O=L(),R(`warn`,`Sending ${O.command_id} to Desktop…`);let e=await f(O);de(O.command_id),z(),R(`good`,`Accepted by Desktop. Task ${e.task_id??`created`}; ${e.state||`queued for governed processing`}.`),await B()}catch(e){let t=e instanceof o;O&&!t&&ue(O),z();let n=e instanceof Error?e.message:`Desktop unavailable`;R(`bad`,t?`${n} Desktop rejected the command; it was not mislabeled as an offline save.`:`${n}${O?` The command envelope was saved locally.`:``}`)}}),E(`save-command`).addEventListener(`click`,()=>{try{O=L(),ue(O),z(),R(`good`,`${O.command_id} saved locally.`)}catch(e){R(`bad`,e instanceof Error?e.message:`Command could not be saved.`)}}),E(`copy-command`).addEventListener(`click`,async()=>{try{O=L(),await navigator.clipboard.writeText(JSON.stringify(O,null,2)),R(`good`,`${O.command_id} copied as JSON.`)}catch(e){R(`bad`,e instanceof Error?e.message:`Command could not be copied.`)}});var B=async()=>{let e=E(`desktop-pill`),t=E(`desktop-state`),n=E(`merovingian-state`),r=E(`desktop-pill-text`),i=E(`desktop-tasks`);e.dataset.state=`checking`,r.textContent=`checking local runtime`,t.textContent=`Checking`,n.textContent=`Checking`;try{let a=await c();k=a.authority_contract||null,N=a.auth?.username||N,E(`owner-username`).value=N,j||(E(`owner-auth-state`).textContent=a.auth?.configured?a.auth.must_change?`First change required`:`Sign-in required`:`Credential setup held`,a.auth?.configured?a.auth.must_change&&P(`warn`,`Sign in with the bootstrap password, then complete the required change.`):P(`bad`,`The dedicated owner credential is not configured on Desktop.`)),I(),e.dataset.state=a.ok?`online`:`degraded`,r.textContent=a.ok?`local runtime connected`:`runtime connected; health needs review`,t.textContent=`Online`,n.textContent=a.merovingian?.status===`pass`?`Healthy`:`Degraded`;try{let e=await p();i.innerHTML=e.length?e.map(e=>`<article class="task-card"><div><strong>${b(String(e.title||`Untitled task`))}</strong><span>${b(String(e.status||`unknown`))} · ${b(String(e.priority||`normal`))}</span><small>Task ${b(String(e.id||``))}</small></div></article>`).join(``):`<p class="muted">Desktop queue is clear.</p>`}catch{i.innerHTML=`<p class="muted">Desktop is online, but task listing is unavailable.</p>`}try{$e((await m()).projects)}catch{$e([])}try{tt(await ae())}catch{tt([])}try{et(await re(),a.auth?.configured===!0)}catch(e){E(`result-auth-state`).textContent=a.auth?.configured===!0?`Owner gate configured`:`Content gate held`,E(`desktop-results`).innerHTML=C(e instanceof Error?e.message:`Local result metadata is unavailable.`)}try{nt(await ce())}catch(e){A=[];let t=a.representation_edge?.enabled===!1?`Canonical representation objects are intentionally disabled by the current launch gate.`:e instanceof Error?e.message:`Representation objects are unavailable.`;E(`representation-graphs`).innerHTML=`<article class="panel"><p class="eyebrow">Objects unavailable</p><h2>Feature-gated, not empty</h2><p class="muted">${b(t)}</p></article>`}}catch{k=null,I(),e.dataset.state=`offline`,r.textContent=`start LightSpeed Desktop and the local bridge`,t.textContent=`Offline`,n.textContent=`Offline`,E(`project-count`).textContent=`0`,i.innerHTML=`<p class="muted">Desktop is offline. Commands can still be saved, copied or downloaded.</p>`,E(`desktop-projects`).innerHTML=`<p class="muted">Project registry unavailable while Desktop is offline.</p>`,E(`desktop-reviews`).innerHTML=`<p class="muted">Review queue unavailable while Desktop is offline.</p>`,E(`owner-auth-state`).textContent=`Desktop offline`,P(`bad`,`Start the local Desktop bridge before signing in.`),E(`result-auth-state`).textContent=`Desktop offline`,E(`desktop-results`).innerHTML=`<p class="muted">Fixed local result metadata is unavailable while Desktop is offline.</p>`,nt([])}};E(`refresh-desktop`).addEventListener(`click`,()=>void B()),z(),B();var rt=E(`neo-exchange`),it=new URL(`./data/neo_exchange.json`,document.baseURI).toString();Ce(async()=>{let e=await fetch(it,{cache:`no-store`});if(!e.ok)throw Error(`Neo exchange returned HTTP ${e.status}`);return e.json()}).then(e=>{rt.innerHTML=Te(e)}),document.querySelectorAll(`.tab`).forEach(e=>e.addEventListener(`click`,()=>{document.querySelectorAll(`.tab`).forEach(e=>e.classList.remove(`active`)),document.querySelectorAll(`.view`).forEach(e=>e.classList.remove(`active`)),e.classList.add(`active`),E(`view-${e.dataset.view}`).classList.add(`active`)}));var at=new URL(`./data/site_integration.json`,document.baseURI).toString(),ot=async()=>{try{let e=await fetch(at,{cache:`no-store`});return e.ok?await e.json():null}catch{return null}},st=()=>{if(document.getElementById(`lsgo-sites-side-edit-styles`))return;let e=document.createElement(`style`);e.id=`lsgo-sites-side-edit-styles`,e.textContent=`
  ...
  `,e.querySelectorAll(`[data-ls-agent]`).forEach(e=>{e.addEventListener(`click`,()=>Z(e.dataset.lsAgent,!0))}),e.querySelector(`[data-ls-home]`)?.addEventListener(`click`,()=>X(`command`)),e.querySelector(`[data-ls-focus]`)?.addEventListener(`click`,()=>{let e=!q.focusMode;J({focusMode:e}),document.body.dataset.lsFocus=e?`true`:`false`}),e},_t=()=>{let e=document.createElement(`section`);return e.className=`ls-os-status-strip`,e.setAttribute(`aria-label`,`LightSpeed operating state`),e.innerHTML=`
  ...
  `,e},vt=()=>{let e=document.createElement(`section`);return e.className=`ls-os-workflow`,e.setAttribute(`aria-label`,`ACR3 operating workflow`),e.innerHTML=`
  ...
  `,e.querySelectorAll(`[data-ls-stage]`).forEach(e=>{e.addEventListener(`click`,()=>Q(e.dataset.lsStage))}),e},yt=()=>{let e=document.createElement(`div`);return e.className=`ls-os-palette`,e.id=`ls-os-palette`,e.hidden=!0,e.innerHTML=`
