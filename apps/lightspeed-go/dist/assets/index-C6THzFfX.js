(function(){let e=document.createElement(`link`).relList;if(e&&e.supports&&e.supports(`modulepreload`))return;for(let e of document.querySelectorAll(`link[rel="modulepreload"]`))n(e);new MutationObserver(e=>{for(let t of e)if(t.type===`childList`)for(let e of t.addedNodes)e.tagName===`LINK`&&e.rel===`modulepreload`&&n(e)}).observe(document,{childList:!0,subtree:!0});function t(e){let t={};return e.integrity&&(t.integrity=e.integrity),e.referrerPolicy&&(t.referrerPolicy=e.referrerPolicy),e.crossOrigin===`use-credentials`?t.credentials=`include`:e.crossOrigin===`anonymous`?t.credentials=`omit`:t.credentials=`same-origin`,t}function n(e){if(e.ep)return;e.ep=!0;let n=t(e);fetch(e.href,n)}})();var e=`lightspeed-go-command-v2`,t=`http://127.0.0.1:8765`,n=(e=>{let n=e?.trim();if(!n)return t;let r;try{r=new URL(n)}catch{throw TypeError(`VITE_LIGHTSPEED_DESKTOP_ORIGIN must be an absolute URL`)}let i=new Set([`127.0.0.1`,`localhost`,`::1`,`[::1]`]).has(r.hostname.toLowerCase());if(r.protocol!==`https:`&&!(r.protocol===`http:`&&i))throw TypeError(`Remote LightSpeed Desktop origins must use HTTPS`);if(r.username||r.password)throw TypeError(`LightSpeed Desktop origins must not contain credentials`);if(r.pathname!==`/`||r.search||r.hash)throw TypeError(`LightSpeed Desktop origins must not contain a path, query or fragment`);return r.origin})(void 0),r=[`Achilles`,`Neo`,`Architect`,`TheConstruct`,`Morpheus`,`Oracle`,`Smith`,`Merovingian`,`Trinity`],i=(e,t)=>e.replace(/\s+/g,` `).trim().slice(0,t),a=e=>{let t=e.toLowerCase();return/\b(ui|site|web|design|visual|layout|canva|accessibility)\b/.test(t)?`Trinity`:/\b(git|github|code|build|commit|branch|deploy|schema|api|runtime)\b/.test(t)?`Smith`:/\b(source|evidence|research|data|document|drive|sheet|workbook|citation)\b/.test(t)?`Oracle`:/\b(proof|claim|verify|conflict|confidence|audit)\b/.test(t)?`Morpheus`:/\b(simulate|simulation|model|gmat|trajectory|twin|physics)\b/.test(t)?`TheConstruct`:/\b(plan|mission|architecture|dependency|roadmap|system|project)\b/.test(t)?`Architect`:/\b(health|status|diagnostic|failure|error|monitor|storage|cleanup|archive)\b/.test(t)?`Merovingian`:/\b(coordinate|queue|handoff|agent|task|execute|run)\b/.test(t)?`Neo`:`Achilles`},o=t=>{let n=i(t.instruction,4e3);if(!n)throw TypeError(`instruction is required`);let r=new Date().toISOString(),o=Math.random().toString(36).slice(2,8).toUpperCase(),s=t.targetFloor??a(n),c=t.authorityContract;if(!c)throw TypeError(`Desktop authority contract is not available`);let l=i(c.canonical_gate_id,160),u=i(c.owner_decision_ref,160),d=i(c.core_acceptance_ref,160),f=i(c.approval_or_hold_state,40).toLowerCase(),p=i(c.authorised_scope,1e3),m=i(c.prohibited_scope,1e3);if(!l||!u||!d||!p||!m)throw TypeError(`Desktop authority contract is incomplete`);if(![`approve`,`approved`,`operator_approved`,`operator_authorized`,`operator_authorised`].includes(f))throw TypeError(`Desktop authority contract is held`);let h=t.executionMode??`review`;return{schema_version:e,command_id:`LSGO-${r.replace(/\D/g,``).slice(0,14)}-${o}`,created_utc:r,source:`LS GO`,title:i(t.title||n,160),instruction:n,target_floor:s,oversight_floor:`Achilles`,priority:t.priority??`normal`,execution_mode:h,action_type:t.actionType??`cognigrex_workflow`,proof_required:!0,public_safe:!0,canonical_gate_id:l,owner_decision_ref:u,core_acceptance_ref:d,approval_or_hold_state:f,authorised_scope:p,prohibited_scope:m,requested_scope:`${s} private local ${h} queue`}},s=class extends Error{status;constructor(e,t){super(t),this.name=`DesktopRequestError`,this.status=e}},c=async(e,t,n=3500)=>{let r=new AbortController,i=window.setTimeout(()=>r.abort(),n);try{let n=await fetch(e,{...t,signal:r.signal,cache:`no-store`});if(!n.ok){let e=`Desktop returned HTTP ${n.status}`;try{let t=await n.json();t.detail&&(e=t.detail)}catch{}throw new s(n.status,e)}return await n.json()}finally{window.clearTimeout(i)}},l=(e=n)=>c(`${e}/api/v1/status`,{method:`GET`},1e4),u=(e,t,r=n)=>c(`${r}/api/v1/auth/login`,{method:`POST`,headers:{"Content-Type":`application/json`},body:JSON.stringify({username:i(e,64),password:t.slice(0,1024)})},15e3),d=(e,t,r,a,o,s=n)=>c(`${s}/api/v1/auth/change-password`,{method:`POST`,headers:{"Content-Type":`application/json`,[o?`X-LightSpeed-Password-Change`:`X-LightSpeed-Session`]:a.slice(0,256)},body:JSON.stringify({username:i(e,64),current_password:t.slice(0,1024),new_password:r.slice(0,1024)})},2e4),f=(e,t=n)=>c(`${t}/api/v1/auth/logout`,{method:`POST`,headers:{"X-LightSpeed-Session":e.slice(0,256)}},7e3),p=(e,t=n)=>c(`${t}/api/v1/ls-go/commands`,{method:`POST`,headers:{"Content-Type":`application/json`},body:JSON.stringify(e)},7e3),m=async(e=n)=>{let t=await c(`${e}/api/v1/tasks?limit=12`,{method:`GET`});return Array.isArray(t.tasks)?t.tasks:[]},h=async(e=n)=>{let t=await c(`${e}/api/v1/projects`,{method:`GET`},15e3);return{projects:Array.isArray(t.projects)?t.projects:[],summary:t.summary||{},duplicateNames:Array.isArray(t.duplicate_names)?t.duplicate_names:[],cleanupSummary:t.cleanup_summary||{}}},ee=(e,t)=>{let n=encodeURIComponent(e);return t===void 0?`/api/v1/projects/${n}/files`:`/api/v1/projects/${n}/files/${t.split(`/`).map(e=>encodeURIComponent(e)).join(`/`)}`},te=async(e,t=n)=>c(`${t}${ee(e)}?limit=200`,{method:`GET`},15e3),ne=async(e,t,r,i=n)=>c(`${i}${ee(e,t)}`,{method:`GET`,headers:{"X-LightSpeed-Session":r.slice(0,256)}},1e4),re=e=>e===void 0?`/api/v1/results`:`/api/v1/results/${encodeURIComponent(e)}`,ie=async(e=n,t=50)=>c(`${e}${re()}?limit=${Math.max(1,Math.min(t,200))}`,{method:`GET`},1e4),ae=async(e,t,r=n)=>c(`${r}${re(e)}`,{method:`GET`,headers:{"X-LightSpeed-Session":t.slice(0,256)}},1e4),oe=async(e=n,t=50)=>{let r=await c(`${e}/api/v1/reviews?limit=${Math.max(1,Math.min(t,200))}`,{method:`GET`},1e4);return Array.isArray(r.reviews)?r.reviews:[]},se=async(e,t,r=``,a=``,o=n)=>c(`${o}/api/v1/reviews/${encodeURIComponent(e)}/decision`,{method:`POST`,headers:{"Content-Type":`application/json`,"X-LightSpeed-Session":a.slice(0,256)},body:JSON.stringify({decision:t,note:i(r,1e3)})},7e3),ce=(e,t,n)=>{let r=n.receipt?.drive_writeback_mode;return r===`owner_approved_exact_drive_target`?`${e} marked ${t}. Owner-approved Drive decision receipt written by Desktop.`:r===`local_outbox_pending_drive_sync`?`${e} marked ${t}. Local outbox receipt staged; Drive sync remains pending.`:`${e} marked ${t}. Decision receipt recorded; destination requires verification.`},le=async(e=n)=>{let t=await c(`${e}/api/v1/representation-graphs`,{method:`GET`},15e3);return Array.isArray(t.graphs)?t.graphs:[]},ue=async(e,t,r,a=[],o=``,s=``,l=n)=>c(`${l}/api/v1/representation-reviews/${encodeURIComponent(e)}/decision`,{method:`POST`,headers:{"Content-Type":`application/json`,"X-LightSpeed-Session":s.slice(0,256)},body:JSON.stringify({decision:t,scope:r,edge_ids:a.slice(0,100),note:i(o,1e3)})},1e4),g=`lightspeed-go-pending-commands-v1`,_=()=>{try{let e=JSON.parse(localStorage.getItem(g)||`[]`);return Array.isArray(e)?e:[]}catch{return[]}},de=e=>{let t=[e,..._().filter(t=>t.command_id!==e.command_id)].slice(0,30);return localStorage.setItem(g,JSON.stringify(t)),t},fe=e=>{let t=_().filter(t=>t.command_id!==e);return localStorage.setItem(g,JSON.stringify(t)),t},pe=e=>{let t=new Blob([JSON.stringify(e,null,2)],{type:`application/json`}),n=URL.createObjectURL(t),r=document.createElement(`a`);r.href=n,r.download=`${e.command_id}.json`,r.click(),URL.revokeObjectURL(n)},me=`lightspeed-neo-exchange-v1`,he=[`critical`,`high`,`normal`,`low`],ge=[`queued`,`active`,`review`,`blocked`,`complete`],_e=[`icon`,`age_label`],v=e=>typeof e==`object`&&!!e&&!Array.isArray(e),y=(e,t,n)=>{if(typeof e!=`string`)return t;let r=e.replace(/\s+/g,` `).trim();return r?r.slice(0,n):t},ve=(e,t,n)=>{let r=y(e,``,n);if(!r)throw TypeError(`queue record ${t} is required`);return r},ye=(e,t,n)=>typeof e==`string`&&t.includes(e)?e:n,be=e=>v(e)?Object.fromEntries(_e.flatMap(t=>{let n=y(e[t],``,48);return n?[[t,n]]:[]})):{},xe=e=>{if(!v(e))throw TypeError(`queue record must be an object`);return{id:ve(e.id,`id`,80),title:ve(e.title,`title`,160),priority:ye(e.priority,he,`normal`),status:ye(e.status,ge,`queued`),source:y(e.source,`GO Gate`,48),target:y(e.target,`Neo`,48),created_utc:y(e.created_utc,``,32),extensions:be(e.extensions),notes:y(e.notes,``,240)}},Se=e=>{if(!v(e))throw TypeError(`Neo exchange must be an object`);let t=Array.isArray(e.queue)?e.queue.map(xe):[];return{schema_version:me,generated_at_utc:y(e.generated_at_utc,``,32),queue:t}},Ce=e=>({total:e.queue.length,critical:e.queue.filter(e=>e.priority===`critical`).length,active:e.queue.filter(e=>e.status!==`complete`).length,complete:e.queue.filter(e=>e.status===`complete`).length}),b=e=>e.replace(/[&<>"']/g,e=>({"&":`&amp;`,"<":`&lt;`,">":`&gt;`,'"':`&quot;`,"'":`&#039;`})[e]??e),we=async e=>{try{return Se(await e())}catch{return Se({})}},Te=e=>e.status===`blocked`?`blocked`:e.status===`complete`?`pass`:e.priority===`critical`||e.priority===`high`?`warn`:`ready`,Ee=e=>{let t=Ce(e),n=e.queue.length?e.queue.map(e=>`
            <li class="status-row ${Te(e)}">
              <div>
...
`,ke=e=>{let t=e.preview,n=`<p class="muted">Metadata only. Binary or non-UTF-8 content is not transferred into LS GO.</p>`;return t.state===`empty`?n=`<p class="muted">The file is empty.</p>`:t.state===`available`&&(n=`<pre>${b(t.text||``)}</pre>${t.truncated?`<small class="muted">Preview truncated at the governed byte limit.</small>`:``}`),`
  ...
  `},Ae=(e,t)=>{e.querySelectorAll(`[data-project-files]`).forEach(e=>{e.addEventListener(`click`,()=>{let n=e.dataset.projectFiles||``;n&&t(n,e)})})},je=(e,t)=>{e.querySelectorAll(`[data-project-file][data-project-id]`).forEach(e=>{e.addEventListener(`click`,()=>{let n=e.dataset.projectId||``,r=e.dataset.projectFile||``;n&&r&&t(n,r,e)})})},Me=e=>{let t=Number(e||0);return t<1024?`${t} B`:t<1024**2?`${(t/1024).toFixed(1)} KB`:`${(t/1024**2).toFixed(1)} MB`},Ne=e=>{let t=String(e.status||`unknown`).toLowerCase();return[`complete`,`completed`,`pass`,`passed`].includes(t)?`good`:[`blocked`,`failed`,`error`].includes(t)?`bad`:`warn`},Pe=(e,t)=>{let n=e.summary,r=`<p class="result-receipt-boundary">${b(e.boundary)}</p>`,i=t?`Exact receipt content requires the local owner-confirmation token.`:`Content inspection is held because the local owner-confirmation token is not configured.`;if(!e.results.length)return`<p class="muted">${e.state===`restricted`?`No eligible fixed receipts are visible; invalid receipt objects remain withheld.`:`No fixed local result receipts have been written yet.`}</p><p class="result-receipt-auth">${i}</p>${r}`;let a=e.results.map(e=>{let n=e.action_type||`untyped`,r=e.target_floor||`floor unknown`,i=e.completed_utc||e.created_utc||e.modified_utc,a=[e.task_id==null?null:`Task ${e.task_id}`,e.job_id==null?null:`Job ${e.job_id}`,e.command_id||null].filter(Boolean).join(` · `),o=t?``:` disabled aria-disabled="true"`;return`
    ...
    ${r}
  `},Fe=e=>{let t=JSON.stringify(e.result,null,2);return`
    <section class="result-receipt-preview">
      <div class="result-receipt-preview-head">
        <strong>${b(e.identity.result_id)}</strong>
        <small>${Me(e.identity.size_bytes)} · SHA-256 ${b(e.identity.sha256)}</small>
      </div>
      <pre>${b(t)}</pre>
  ...
  `},C=e=>`
  <p class="result" data-tone="bad">${b(e)}</p>
`,Ie=(e,t)=>{e.querySelectorAll(`[data-result-receipt]`).forEach(e=>{e.addEventListener(`click`,()=>{let n=e.dataset.resultReceipt||``;n&&!e.disabled&&t(n,e)})})},w=e=>String(e??``).replace(/&/g,`&amp;`).replace(/</g,`&lt;`).replace(/>/g,`&gt;`).replace(/"/g,`&quot;`).replace(/'/g,`&#39;`),T=e=>e?`${e.slice(0,12)}…${e.slice(-8)}`:`not available`,Le=e=>e.path_exposed===!1?String(e.label||`private local evidence`):[e.repository,e.commit_sha?`commit ${String(e.commit_sha).slice(0,12)}`:null,e.path,e.drive_file_id?`Drive ${e.drive_file_id}`:null,e.sheet_name,e.stable_key||e.content_key,e.missing_state].filter(Boolean).map(w).join(` · `)||w(e.locator_type||`logical`),E=e=>w(JSON.stringify(e,null,2)),Re=e=>{let t=e.representations.filter(e=>e.state===`active`).length,n=e.missing.length;return!t&&n?`unable to determine`:n>t?`smaller bowl`:t>n?`larger bowl`:`unchanged bowl`},ze=e=>{let t=e.review,n=t?.review_stage||`identity`,r=e.edges.map(e=>e.edge_id).join(`|`),i=t?`
    ...
    <tr>
      ...
      <article class="horizon-card">
        ...
        <p>${w(e.objective)}</p>
        <details><summary>Assumptions and constraints</summary><pre>${E({assumptions:e.assumptions,constraints:e.constraints})}</pre></details>
      ...
      <article class="horizon-card">
        ...
        <details><summary>Source weight summary</summary><pre>${E(e.source_weight_summary)}</pre></details>
    ...
    <article class="panel graph-panel" data-object-id="${w(e.object.object_id)}">
      ...
      <div class="graph-summary">
        ...
        <div><span>Judgment</span><strong>${Re(e)}</strong></div>
      </div>
      ...
      ${i}
    </article>`},Be=e=>e.length?e.map(ze).join(``):`<article class="panel"><p class="muted">The feature-gated representation edge is disabled or unavailable.</p></article>`,Ve=[[`LightSpeed Git`,`https://github.com/achillesromer-coder/LightSpeed`,`Versioned implementation and receipts`],[`Type 1 Digital Review`,`https://github.com/achillesromer-coder/LightSpeed/pull/46`,`Draft 3D, six-view and T1-00..07 review packages`],[`Type 1 Römer Canon`,`https://docs.google.com/spreadsheets/d/1refNFmebTcmPVojCuZsyILJEWaKz-sVzYLfZtqMl8k8/edit`,`Living programme, evidence, handoff and release gates`],[`ACR3 Handoffs`,`https://docs.google.com/spreadsheets/d/1AgAhLPNtrO91C_-ea7EdOkOsyXrCCYvFVvmDSGq8uls/edit`,`Append-only cross-corpus reconciliation receipts`],[`LS GO Queue`,`https://docs.google.com/spreadsheets/d/1f5i4V3FshYHkztv3_HAg0ZofUl0sdcJZcwrlesUlCfM/edit`,`Phone tasks, approvals, commands, results and sync health`],[`Portfolio Handoff`,`https://docs.google.com/document/d/1tsDkb79UVX_SqS2-oBgc5DHb89QIlH3DcmKMN77hdOo/edit`,`Cross-chat portfolio continuity`],[`Römer Industries`,`https://romer.industries`,`Reviewed public portfolio surface`]],He=[{name:`Central Facility Boundary`,radiusM:2500,description:`All facilities and buildings remain inside this radius.`},{name:`Active Eco-Restoration`,radiusM:3500,description:`1 km active band for managed biome, native rehabilitation, and functional climate pockets.`},{name:`Passive Eco-Restoration`,radiusM:11e3,description:`Outer passive restoration reserve securing the full radial print.`}],Ue=[{id:`integration-hall`,name:`Integration Hall`,footprint:`40 m x 75 m, 22 m clear`,elevation:`+1.5 m floor, +8% beveled foundation`,releaseStatus:`canonical`,notes:`Starship-compatible roller doors both ends; single bridge crane spans the hall.`},{id:`chainhill`,name:`ChainHill Relay`,footprint:`~225-250 m relay length`,elevation:`flat raised concrete, approximately 3 m above water table`,releaseStatus:`bounded-assumption`,notes:`Incoming and outgoing tracks oppose each other with two anti-parallel internal lines.`},{id:`x-pads`,name:`X-Layout Pads`,footprint:`solid pads, no beveled building base`,elevation:`pad-specific hardstand`,releaseStatus:`canonical`,notes:`Flame/exhaust outlets orient away from central node and buildings.`},{id:`mission-control`,name:`Mission Control / ATC`,footprint:`pentagon base, level 1 at 80%, four-storey tower`,elevation:`+1.5 m floor, +8% beveled foundation`,releaseStatus:`canonical`,notes:`Foyer, cafeteria, meeting, offices, emergency access, elevator, and top operating floor.`},{id:`living-rd`,name:`R&D + Living Quarters`,footprint:`room-level workbook detail pending`,elevation:`+1.5 m floor, +8% beveled foundation`,releaseStatus:`known-unknown`,notes:`FIFO and FIFO+family support with ground community and rooftop lifestyle zones.`}],We=[`site_zones`,`facilities`,`rooms_spaces`,`roads_tracks`,`pads_exhaust`,`eco_restoration`,`standards_evidence`,`viewer_toggles`,`known_unknowns`],Ge=document.getElementById(`app`);if(!Ge)throw Error(`LightSpeed Go mount node #app not found.`);Ge.innerHTML=`
  <main class="shell">
    <header class="topbar">
      <div>
    ...
    <section class="view" id="view-system">
      ...
      <div class="two-column">
        ...
        <article class="panel"><p class="eyebrow">Existing twin context</p><h2>Spaceport contract retained</h2><p class="muted">${He.length} zones · ${Ue.length} facility records · ${We.length} workbook tabs. The twin remains bounded context, not the command-centre homepage.</p></article>
      </div>
    </section>

    <section class="view" id="view-sources">
      <div class="source-grid">${Ve.map(([e,t,n])=>`<a class="source-card" href="${t}" target="_blank" rel="noreferrer"><strong>${e}</strong><span>${n}</span><em>Open ↗</em></a>`).join(``)}</div>
...
`;var D=e=>{let t=document.getElementById(e);if(!t)throw Error(`Missing #${e}`);return t},O=D(`instruction`),Ke=D(`target-floor`),qe=D(`priority`),Je=D(`execution-mode`),Ye=D(`route-preview`),Xe=D(`command-result`),k=null,A=null,Ze=[],j=[],M=``,N=``,P=`NCNB`,F=(e,t)=>{let n=D(`owner-auth-result`);n.dataset.tone=e,n.textContent=t},Qe=e=>{let t=D(`owner-login-form`),n=D(`owner-change-form`),r=D(`owner-logout`);if(P=e.credential?.username||P,M=e.session_token||``,N=e.password_change_token||``,D(`owner-password`).value=``,e.authenticated&&M){n.hidden=!0,r.hidden=!1,D(`owner-auth-state`).textContent=`${P} signed in`,F(`good`,`Owner session active until ${e.expires_utc||`page close`}.`);return}r.hidden=!0,n.hidden=!e.change_required,t.hidden=!1,D(`owner-auth-state`).textContent=e.change_required?`Password change required`:`Sign-in required`,F(`warn`,e.change_required?`The bootstrap password was verified. Enter it again with a new password to complete first login.`:`Owner sign-in is required for unredacted files, receipts, and decisions.`)},I=()=>M||(F(`warn`,`Sign in as NCNB before performing this owner-gated action.`),``);D(`owner-login-form`).addEventListener(`submit`,async e=>{e.preventDefault();let t=D(`owner-username`).value.trim(),n=D(`owner-password`);try{Qe(await u(t,n.value))}catch(e){n.value=``,F(`bad`,e instanceof Error?e.message:`Owner sign-in failed.`)}}),D(`owner-change-form`).addEventListener(`submit`,async e=>{e.preventDefault();let t=D(`owner-current-password`),n=D(`owner-new-password`),r=D(`owner-confirm-password`);if(n.value!==r.value){F(`bad`,`The new passwords do not match.`);return}let i=N||M;if(!i){F(`bad`,`Sign in before changing the password.`);return}try{let e=await d(P,t.value,n.value,i,!!N);t.value=``,n.value=``,r.value=``,Qe(e)}catch(e){t.value=``,n.value=``,r.value=``,F(`bad`,e instanceof Error?e.message:`Password change failed.`)}}),D(`owner-logout`).addEventListener(`click`,async()=>{let e=M;if(M=``,N=``,e)try{await f(e)}catch{}D(`owner-logout`).hidden=!0,D(`owner-auth-state`).textContent=`Signed out`,F(`warn`,`Owner session cleared from this page.`)});var L=()=>{let e=a(O.value||`governance`);Ke.value=e;let t=A?.canonical_gate_id;Ye.innerHTML=`<strong>Achilles route:</strong> ${e} is primary. Neo coordinates and proof returns to this gate.${t?` <small>Authority: ${b(t)}</small>`:` <small>Waiting for the Desktop authority contract.</small>`}`};O.addEventListener(`input`,L),L();var R=()=>o({instruction:O.value,targetFloor:Ke.value,priority:qe.value,executionMode:Je.value,authorityContract:A}),z=(e,t)=>{Xe.dataset.tone=e,Xe.textContent=t},B=()=>{let e=_();D(`pending-count`).textContent=String(e.length);let t=D(`pending-commands`);if(!e.length){t.innerHTML=`<p class="muted">No locally saved commands.</p>`;return}t.innerHTML=e.map(e=>`<article class="task-card"><div><strong>${b(e.title)}</strong><span>${b(e.target_floor)} · ${b(e.priority)} · ${b(e.execution_mode)}</span><small>${b(e.command_id)}</small></div><div class="task-actions"><button data-send="${b(e.command_id)}">Send</button><button data-download="${b(e.command_id)}">Download</button></div></article>`).join(``),t.querySelectorAll(`[data-send]`).forEach(t=>t.addEventListener(`click`,async()=>{let n=e.find(e=>e.command_id===t.dataset.send);if(n)try{let e=await p(n);fe(n.command_id),B(),z(`good`,`Desktop accepted ${e.command_id||n.command_id}. Task ${e.task_id??`created`}.`),await V()}catch(e){z(`bad`,e instanceof Error?e.message:`Desktop command failed.`)}})),t.querySelectorAll(`[data-download]`).forEach(t=>t.addEventListener(`click`,()=>{let n=e.find(e=>e.command_id===t.dataset.download);n&&pe(n)}))},$e=e=>{let t=D(`desktop-projects`);D(`project-count`).textContent=String(e.length),t.innerHTML=De(e),Ae(t,async(e,t)=>{let n=t.closest(`[data-project-card]`)?.querySelector(`.project-files`);if(n){if(t.getAttribute(`aria-expanded`)===`true`){t.setAttribute(`aria-expanded`,`false`),t.textContent=`Files`,n.hidden=!0;return}t.disabled=!0,t.textContent=`Loading…`,n.hidden=!1,n.innerHTML=`<p class="muted">Reading bounded project metadata…</p>`;try{n.innerHTML=Oe(await te(e)),je(n,async(e,t,r)=>{let i=n.querySelector(`.project-file-result`);if(!i)return;let a=I();if(!a){i.innerHTML=S(`File preview held: sign in through the NCNB owner gate first.`);return}r.disabled=!0,i.innerHTML=`<p class="muted">Opening read-only result…</p>`;try{i.innerHTML=ke(await ne(e,t,a))}catch(e){i.innerHTML=S(e instanceof Error?e.message:`Project file result is unavailable.`)}finally{r.disabled=!1}}),t.setAttribute(`aria-expanded`,`true`),t.textContent=`Hide files`}catch(e){n.innerHTML=S(e instanceof Error?e.message:`Project files are unavailable.`),t.textContent=`Retry files`}finally{t.disabled=!1}}})},et=(e,t)=>{let n=D(`desktop-results`),r=D(`result-auth-state`);r.textContent=t?`Owner gate configured`:`Content gate held`,n.innerHTML=Pe(e,t),Ie(n,async(e,r)=>{let i=n.querySelector(`.result-receipt-detail`);if(!i)return;if(!t){i.innerHTML=C(`Receipt content is held because owner confirmation is not configured on the local bridge.`);return}let a=I();if(!a){i.innerHTML=C(`Receipt inspection held: sign in through the NCNB owner gate first.`);return}r.disabled=!0,i.innerHTML=`<p class="muted">Opening owner-confirmed read-only receipt…</p>`;try{i.innerHTML=Fe(await ae(e,a))}catch(e){i.innerHTML=C(e instanceof Error?e.message:`Local result receipt is unavailable.`)}finally{r.disabled=!1}})},tt=e=>{Ze=e;let t=D(`desktop-reviews`);if(!e.length){t.innerHTML=`<p class="muted">No project receipts are awaiting review.</p>`;return}t.innerHTML=e.slice(0,30).map(e=>{let t=e.state||`pending_review`,n=t===`pending_review`?`<div class="task-actions"><button data-review="${b(e.review_id)}" data-decision="approve">Approve</button><button data-review="${b(e.review_id)}" data-decision="hold">Hold</button><button data-review="${b(e.review_id)}" data-decision="reject">Reject</button></div>`:``;return`<article class="task-card"><div><strong>${b(e.title||`Project receipt`)}</strong><span>${b(t)} · ${b(e.event_type||`receipt`)}</span><small>${b(e.summary||e.review_id)}</small></div>${n}</article>`}).join(``),t.querySelectorAll(`[data-review]`).forEach(e=>e.addEventListener(`click`,async()=>{let t=e.dataset.review||``,n=e.dataset.decision,r=Ze.find(e=>e.review_id===t);if(!r||!t)return;let i=window.prompt(`${n.toUpperCase()}: ${r.title||t}\nOptional decision note:`,``)??``,a=I();if(!a){z(`bad`,`Review decision held: sign in through the NCNB owner gate first.`);return}try{z(`good`,ce(t,n,await se(t,n,i,a))),await V()}catch(e){z(`bad`,e instanceof Error?e.message:`Review decision failed.`)}}))},nt=e=>{j=e;let t=D(`representation-graphs`);t.innerHTML=Be(e),t.querySelectorAll(`[data-representation-review]`).forEach(e=>{e.addEventListener(`click`,async()=>{let t=e.dataset.representationReview||``,n=e.dataset.decision,r=e.dataset.scope||`identity`,i=r===`edges`?(e.dataset.edgeIds||``).split(`|`).filter(Boolean).slice(0,100):[],a=j.find(e=>e.review?.review_id===t);if(!t||!a)return;let o=window.prompt(`${n.replace(/_/g,` `).toUpperCase()}: ${a.object.display_name}\n${r===`identity`?`Identity is reviewed before edges.`:`${i.length} bounded edges selected.`}\nOptional decision note:`,``)??``,s=I();if(!s){z(`bad`,`Representation decision held: sign in through the NCNB owner gate first.`);return}try{await ue(t,n,r,i,o,s),z(`good`,`${t} recorded ${n}; local staging remains noncanonical until Drive readback.`),await V()}catch(e){z(`bad`,e instanceof Error?e.message:`Representation review decision failed.`)}})})};D(`command-form`).addEventListener(`submit`,async e=>{e.preventDefault(),k=null;try{k=R(),z(`warn`,`Sending ${k.command_id} to Desktop…`);let e=await p(k);fe(k.command_id),B(),z(`good`,`Accepted by Desktop. Task ${e.task_id??`created`}; ${e.state||`queued for governed processing`}.`),await V()}catch(e){let t=e instanceof s;k&&!t&&de(k),B();let n=e instanceof Error?e.message:`Desktop unavailable`;z(`bad`,t?`${n} Desktop rejected the command; it was not mislabeled as an offline save.`:`${n}${k?` The command envelope was saved locally.`:``}`)}}),D(`save-command`).addEventListener(`click`,()=>{try{k=R(),de(k),B(),z(`good`,`${k.command_id} saved locally.`)}catch(e){z(`bad`,e instanceof Error?e.message:`Command could not be saved.`)}}),D(`copy-command`).addEventListener(`click`,async()=>{try{k=R(),await navigator.clipboard.writeText(JSON.stringify(k,null,2)),z(`good`,`${k.command_id} copied as JSON.`)}catch(e){z(`bad`,e instanceof Error?e.message:`Command could not be copied.`)}});var V=async()=>{let e=D(`desktop-pill`),t=D(`desktop-state`),n=D(`merovingian-state`),r=D(`desktop-pill-text`),i=D(`desktop-tasks`);e.dataset.state=`checking`,r.textContent=`checking local runtime`,t.textContent=`Checking`,n.textContent=`Checking`;try{let a=await l();A=a.authority_contract||null,P=a.auth?.username||P,D(`owner-username`).value=P,M||(D(`owner-auth-state`).textContent=a.auth?.configured?a.auth.must_change?`First change required`:`Sign-in required`:`Credential setup held`,a.auth?.configured?a.auth.must_change&&F(`warn`,`Sign in with the bootstrap password, then complete the required change.`):F(`bad`,`The dedicated owner credential is not configured on Desktop.`)),L(),e.dataset.state=a.ok?`online`:`degraded`,r.textContent=a.ok?`local runtime connected`:`runtime connected; health needs review`,t.textContent=`Online`,n.textContent=a.merovingian?.status===`pass`?`Healthy`:`Degraded`;try{let e=await m();i.innerHTML=e.length?e.map(e=>`<article class="task-card"><div><strong>${b(String(e.title||`Untitled task`))}</strong><span>${b(String(e.status||`unknown`))} · ${b(String(e.priority||`normal`))}</span><small>Task ${b(String(e.id||``))}</small></div></article>`).join(``):`<p class="muted">Desktop queue is clear.</p>`}catch{i.innerHTML=`<p class="muted">Desktop is online, but task listing is unavailable.</p>`}try{$e((await h()).projects)}catch{$e([])}try{tt(await oe())}catch{tt([])}try{et(await ie(),a.auth?.configured===!0)}catch(e){D(`result-auth-state`).textContent=a.auth?.configured===!0?`Owner gate configured`:`Content gate held`,D(`desktop-results`).innerHTML=C(e instanceof Error?e.message:`Local result metadata is unavailable.`)}try{nt(await le())}catch(e){j=[];let t=a.representation_edge?.enabled===!1?`Canonical representation objects are intentionally disabled by the current launch gate.`:e instanceof Error?e.message:`Representation objects are unavailable.`;D(`representation-graphs`).innerHTML=`<article class="panel"><p class="eyebrow">Objects unavailable</p><h2>Feature-gated, not empty</h2><p class="muted">${b(t)}</p></article>`}}catch{A=null,L(),e.dataset.state=`offline`,r.textContent=`start LightSpeed Desktop and the local bridge`,t.textContent=`Offline`,n.textContent=`Offline`,D(`project-count`).textContent=`0`,i.innerHTML=`<p class="muted">Desktop is offline. Commands can still be saved, copied or downloaded.</p>`,D(`desktop-projects`).innerHTML=`<p class="muted">Project registry unavailable while Desktop is offline.</p>`,D(`desktop-reviews`).innerHTML=`<p class="muted">Review queue unavailable while Desktop is offline.</p>`,D(`owner-auth-state`).textContent=`Desktop offline`,F(`bad`,`Start the local Desktop bridge before signing in.`),D(`result-auth-state`).textContent=`Desktop offline`,D(`desktop-results`).innerHTML=`<p class="muted">Fixed local result metadata is unavailable while Desktop is offline.</p>`,nt([])}};D(`refresh-desktop`).addEventListener(`click`,()=>void V()),B(),V();var rt=D(`neo-exchange`),it=new URL(`./data/neo_exchange.json`,document.baseURI).toString();we(async()=>{let e=await fetch(it,{cache:`no-store`});if(!e.ok)throw Error(`Neo exchange returned HTTP ${e.status}`);return e.json()}).then(e=>{rt.innerHTML=Ee(e)}),document.querySelectorAll(`.tab`).forEach(e=>e.addEventListener(`click`,()=>{document.querySelectorAll(`.tab`).forEach(e=>e.classList.remove(`active`)),document.querySelectorAll(`.view`).forEach(e=>e.classList.remove(`active`)),e.classList.add(`active`),D(`view-${e.dataset.view}`).classList.add(`active`)}));var at=new URL(`./data/site_integration.json`,document.baseURI).toString(),ot=async()=>{try{let e=await fetch(at,{cache:`no-store`});return e.ok?await e.json():null}catch{return null}},st=()=>{if(document.getElementById(`lsgo-sites-side-edit-styles`))return;let e=document.createElement(`style`);e.id=`lsgo-sites-side-edit-styles`,e.textContent=`
  ...
  `,document.head.appendChild(e)},ct=async()=>{let e=document.querySelector(`.topbar > div:first-child`),t=document.querySelector(`#view-sources`);if(!e||!t)return!1;st();let n=(await ot())?.authority_chain??[`Nathaniel Bouwer`,`Achilles / GO gate`,`agent floor`,`LightSpeed Desktop`,`Git and Drive receipts`];if(!document.getElementById(`site-context-strip`)){let t=document.createElement(`div`);t.id=`site-context-strip`,t.className=`site-context-strip`,t.innerHTML=`
  ...
  `,e.querySelectorAll(`[data-ls-agent]`).forEach(e=>{e.addEventListener(`click`,()=>Z(e.dataset.lsAgent,!0))}),e.querySelector(`[data-ls-home]`)?.addEventListener(`click`,()=>X(`command`)),e.querySelector(`[data-ls-focus]`)?.addEventListener(`click`,()=>{let e=!q.focusMode;J({focusMode:e}),document.body.dataset.lsFocus=e?`true`:`false`}),e},vt=()=>{let e=document.createElement(`section`);return e.className=`ls-os-status-strip`,e.setAttribute(`aria-label`,`LightSpeed operating state`),e.innerHTML=`
  ...
  `,e},yt=()=>{let e=document.createElement(`section`);return e.className=`ls-os-workflow`,e.setAttribute(`aria-label`,`ACR3 operating workflow`),e.innerHTML=`
  ...
  `,e.querySelectorAll(`[data-ls-stage]`).forEach(e=>{e.addEventListener(`click`,()=>Q(e.dataset.lsStage))}),e},bt=()=>{let e=document.createElement(`div`);return e.className=`ls-os-palette`,e.id=`ls-os-palette`,e.hidden=!0,e.innerHTML=`
    <div class="ls-os-palette-card" role="dialog" aria-modal="true" aria-label="LightSpeed command palette">
      <div class="ls-os-palette-head"><strong>LightSpeed</strong><span>${dt}</span><button type="button" data-ls-close aria-label="Close">×</button></div>
