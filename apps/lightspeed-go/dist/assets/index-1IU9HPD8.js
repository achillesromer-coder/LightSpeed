(function(){let e=document.createElement(`link`).relList;if(e&&e.supports&&e.supports(`modulepreload`))return;for(let e of document.querySelectorAll(`link[rel="modulepreload"]`))n(e);new MutationObserver(e=>{for(let t of e)if(t.type===`childList`)for(let e of t.addedNodes)e.tagName===`LINK`&&e.rel===`modulepreload`&&n(e)}).observe(document,{childList:!0,subtree:!0});function t(e){let t={};return e.integrity&&(t.integrity=e.integrity),e.referrerPolicy&&(t.referrerPolicy=e.referrerPolicy),e.crossOrigin===`use-credentials`?t.credentials=`include`:e.crossOrigin===`anonymous`?t.credentials=`omit`:t.credentials=`same-origin`,t}function n(e){if(e.ep)return;e.ep=!0;let n=t(e);fetch(e.href,n)}})();var e=`lightspeed-go-command-v1`,t=`http://127.0.0.1:8765`,n=[`Achilles`,`Neo`,`Architect`,`TheConstruct`,`Morpheus`,`Oracle`,`Smith`,`Merovingian`,`Trinity`],r=(e,t)=>e.replace(/\s+/g,` `).trim().slice(0,t),i=e=>{let t=e.toLowerCase();return/\b(ui|site|web|design|visual|layout|canva|accessibility)\b/.test(t)?`Trinity`:/\b(git|github|code|build|commit|branch|deploy|schema|api|runtime)\b/.test(t)?`Smith`:/\b(source|evidence|research|data|document|drive|sheet|workbook|citation)\b/.test(t)?`Oracle`:/\b(proof|claim|verify|conflict|confidence|audit)\b/.test(t)?`Morpheus`:/\b(simulate|simulation|model|gmat|trajectory|twin|physics)\b/.test(t)?`TheConstruct`:/\b(plan|mission|architecture|dependency|roadmap|system|project)\b/.test(t)?`Architect`:/\b(health|status|diagnostic|failure|error|monitor|storage|cleanup|archive)\b/.test(t)?`Merovingian`:/\b(coordinate|queue|handoff|agent|task|execute|run)\b/.test(t)?`Neo`:`Achilles`},a=t=>{let n=r(t.instruction,4e3);if(!n)throw TypeError(`instruction is required`);let a=new Date().toISOString(),o=Math.random().toString(36).slice(2,8).toUpperCase(),s=t.targetFloor??i(n);return{schema_version:e,command_id:`LSGO-${a.replace(/\D/g,``).slice(0,14)}-${o}`,created_utc:a,source:`LS GO`,title:r(t.title||n,160),instruction:n,target_floor:s,oversight_floor:`Achilles`,priority:t.priority??`normal`,execution_mode:t.executionMode??`review`,proof_required:!0,public_safe:!0}},o=async(e,t,n=3500)=>{let r=new AbortController,i=window.setTimeout(()=>r.abort(),n);try{let n=await fetch(e,{...t,signal:r.signal,cache:`no-store`});if(!n.ok){let e=`Desktop returned HTTP ${n.status}`;try{let t=await n.json();t.detail&&(e=t.detail)}catch{}throw Error(e)}return await n.json()}finally{window.clearTimeout(i)}},s=(e=t)=>o(`${e}/api/v1/status`,{method:`GET`},1e4),c=(e,n=t)=>o(`${n}/api/v1/ls-go/commands`,{method:`POST`,headers:{"Content-Type":`application/json`},body:JSON.stringify(e)},7e3),l=async(e=t)=>{let n=await o(`${e}/api/v1/tasks?limit=12`,{method:`GET`});return Array.isArray(n.tasks)?n.tasks:[]},u=async(e=t)=>{let n=await o(`${e}/api/v1/projects`,{method:`GET`},15e3);return{projects:Array.isArray(n.projects)?n.projects:[],summary:n.summary||{},duplicateNames:Array.isArray(n.duplicate_names)?n.duplicate_names:[],cleanupSummary:n.cleanup_summary||{}}},d=async(e=t,n=50)=>{let r=await o(`${e}/api/v1/reviews?limit=${Math.max(1,Math.min(n,200))}`,{method:`GET`},1e4);return Array.isArray(r.reviews)?r.reviews:[]},f=async(e,n,i=``,a=t)=>o(`${a}/api/v1/reviews/${encodeURIComponent(e)}/decision`,{method:`POST`,headers:{"Content-Type":`application/json`},body:JSON.stringify({decision:n,note:r(i,1e3)})},7e3),ee=async(e=t)=>{let n=await o(`${e}/api/v1/representation-graphs`,{method:`GET`},15e3);return Array.isArray(n.graphs)?n.graphs:[]},te=async(e,n,i,a=[],s=``,c=``,l=t)=>o(`${l}/api/v1/representation-reviews/${encodeURIComponent(e)}/decision`,{method:`POST`,headers:{"Content-Type":`application/json`,"X-LightSpeed-Owner-Confirmation":c.slice(0,256)},body:JSON.stringify({decision:n,scope:i,edge_ids:a.slice(0,100),note:r(s,1e3)})},1e4),p=`lightspeed-go-pending-commands-v1`,m=()=>{try{let e=JSON.parse(localStorage.getItem(p)||`[]`);return Array.isArray(e)?e:[]}catch{return[]}},h=e=>{let t=[e,...m().filter(t=>t.command_id!==e.command_id)].slice(0,30);return localStorage.setItem(p,JSON.stringify(t)),t},g=e=>{let t=m().filter(t=>t.command_id!==e);return localStorage.setItem(p,JSON.stringify(t)),t},_=e=>{let t=new Blob([JSON.stringify(e,null,2)],{type:`application/json`}),n=URL.createObjectURL(t),r=document.createElement(`a`);r.href=n,r.download=`${e.command_id}.json`,r.click(),URL.revokeObjectURL(n)},v=`lightspeed-neo-exchange-v1`,y=[`critical`,`high`,`normal`,`low`],ne=[`queued`,`active`,`review`,`blocked`,`complete`],re=[`icon`,`age_label`],b=e=>typeof e==`object`&&!!e&&!Array.isArray(e),x=(e,t,n)=>{if(typeof e!=`string`)return t;let r=e.replace(/\s+/g,` `).trim();return r?r.slice(0,n):t},S=(e,t,n)=>{let r=x(e,``,n);if(!r)throw TypeError(`queue record ${t} is required`);return r},C=(e,t,n)=>typeof e==`string`&&t.includes(e)?e:n,w=e=>b(e)?Object.fromEntries(re.flatMap(t=>{let n=x(e[t],``,48);return n?[[t,n]]:[]})):{},T=e=>{if(!b(e))throw TypeError(`queue record must be an object`);return{id:S(e.id,`id`,80),title:S(e.title,`title`,160),priority:C(e.priority,y,`normal`),status:C(e.status,ne,`queued`),source:x(e.source,`GO Gate`,48),target:x(e.target,`Neo`,48),created_utc:x(e.created_utc,``,32),extensions:w(e.extensions),notes:x(e.notes,``,240)}},E=e=>{if(!b(e))throw TypeError(`Neo exchange must be an object`);let t=Array.isArray(e.queue)?e.queue.map(T):[];return{schema_version:v,generated_at_utc:x(e.generated_at_utc,``,32),queue:t}},ie=e=>({total:e.queue.length,critical:e.queue.filter(e=>e.priority===`critical`).length,active:e.queue.filter(e=>e.status!==`complete`).length,complete:e.queue.filter(e=>e.status===`complete`).length}),D=e=>e.replace(/[&<>"']/g,e=>({"&":`&amp;`,"<":`&lt;`,">":`&gt;`,'"':`&quot;`,"'":`&#039;`})[e]??e),ae=async e=>{try{return E(await e())}catch{return E({})}},oe=e=>e.status===`blocked`?`blocked`:e.status===`complete`?`pass`:e.priority===`critical`||e.priority===`high`?`warn`:`ready`,se=e=>{let t=ie(e),n=e.queue.length?e.queue.map(e=>`
  ...
  `},O=e=>String(e??``).replace(/&/g,`&amp;`).replace(/</g,`&lt;`).replace(/>/g,`&gt;`).replace(/"/g,`&quot;`).replace(/'/g,`&#39;`),k=e=>e?`${e.slice(0,12)}…${e.slice(-8)}`:`not available`,ce=e=>e.path_exposed===!1?String(e.label||`private local evidence`):[e.repository,e.commit_sha?`commit ${String(e.commit_sha).slice(0,12)}`:null,e.path,e.drive_file_id?`Drive ${e.drive_file_id}`:null,e.sheet_name,e.stable_key||e.content_key,e.missing_state].filter(Boolean).map(O).join(` · `)||O(e.locator_type||`logical`),A=e=>O(JSON.stringify(e,null,2)),le=e=>{let t=e.representations.filter(e=>e.state===`active`).length,n=e.missing.length;return!t&&n?`unable to determine`:n>t?`smaller bowl`:t>n?`larger bowl`:`unchanged bowl`},ue=e=>{let t=e.review,n=t?.review_stage||`identity`,r=e.edges.map(e=>e.edge_id).join(`|`),i=t?`
    <div class="graph-actions" data-review-stage="${O(n)}">
      <strong>${n===`identity`?`Review identity first`:`Review ${e.edges.length} bounded edges`}</strong>
      <div class="task-actions">
        <button data-representation-review="${O(t.review_id)}" data-scope="${n}" data-edge-ids="${O(r)}" data-decision="approve">Approve</button>
        <button data-representation-review="${O(t.review_id)}" data-scope="${n}" data-edge-ids="${O(r)}" data-decision="provisional_approve">Provisional</button>
        <button data-representation-review="${O(t.review_id)}" data-scope="${n}" data-edge-ids="${O(r)}" data-decision="hold">Hold</button>
        <button data-representation-review="${O(t.review_id)}" data-scope="${n}" data-edge-ids="${O(r)}" data-decision="request_evidence">Request evidence</button>
        <button data-representation-review="${O(t.review_id)}" data-scope="${n}" data-edge-ids="${O(r)}" data-decision="reject">Reject</button>
        <button data-representation-review="${O(t.review_id)}" data-scope="${n}" data-edge-ids="${O(r)}" data-decision="supersede">Supersede</button>
      </div>
      <small>${O(t.state)} · graph ${k(t.graph_sha256)}</small>
    </div>`:`<p class="muted">Review packet has not been staged.</p>`,a=e.representations.map(e=>`
    <tr>
      <td><strong>${O(e.representation_type)}</strong><small>${O(e.representation_id)}</small></td>
      <td>${ce(e.locator)}</td>
      <td>${O(e.source_authority)}</td>
      <td>${k(e.content_sha256)}</td>
      <td>${O(e.confidence_class)} (${Math.round(e.confidence_numeric*100)}%)</td>
      <td><span class="state-chip" data-state="${O(e.state)}">${O(e.state)}</span></td>
      <td>${O(e.claim_boundary)}</td>
    </tr>`).join(``),o=e.edges.map(e=>`
    <tr>
      <td>${O(e.from_representation_id)}</td>
      <td><strong>${O(e.relation)}</strong></td>
      <td>${O(e.to_representation_id)}</td>
      <td>${O(e.evidence_bundle_id||`not required`)}</td>
      <td>${O(e.review_state)}</td>
      <td>${O(e.claim_boundary)}</td>
    </tr>`).join(``),s=e.missing.length?e.missing.map(e=>`
      <article class="missing-card">
        <strong>${O(e.type)} · ${O(e.missing_state)}</strong>
        <span><b>Why:</b> ${O(e.reason)}</span>
        <span><b>Next:</b> ${O(e.next_evidence_action)}</span>
        <small>Last search: ${O(e.last_search||`not recorded`)} · Floor: ${O(e.assigned_floor)} · Effect: ${O(e.dependency_effect)}</small>
      </article>`).join(``):`<p class="muted">No required representation is currently missing.</p>`,c=e.conflicts.length?e.conflicts.map(e=>`<article class="missing-card conflict"><strong>${O(e.edge_id)}</strong><span>${O(e.claim_boundary)}</span></article>`).join(``):`<p class="muted">No representation conflict is recorded.</p>`,l=e.horizons.length?e.horizons.map(e=>`
      <article class="horizon-card">
        <div><strong>${O(e.name)}</strong><span>${O(e.state)} · ${O(e.horizon_type)}</span></div>
        <p>${O(e.objective)}</p>
        <details><summary>Assumptions and constraints</summary><pre>${A({assumptions:e.assumptions,constraints:e.constraints})}</pre></details>
        <small>Input ${k(e.input_set_sha256)}</small>
        ...
        <strong>${O(t.display_name)} · ${O(t.state)}</strong>
        <span><b>Object:</b> ${O(t.object_id)}</span>
        <span><b>Identifiers:</b> ${O(n||`none recorded`)}</span>
      </article>`}).join(``):`<p class="muted">No linked object identity is included.</p>`,f=(e.evidence_bundles||[]).length?(e.evidence_bundles||[]).map(e=>`
      <article class="horizon-card">
        <div><strong>${O(e.title)}</strong><span>${O(e.state)}</span></div>
        <p>${O(e.claim_boundary)}</p>
        ...
        <details><summary>Source weight summary</summary><pre>${A(e.source_weight_summary)}</pre></details>
      </article>`).join(``):`<p class="muted">No evidence bundle is linked.</p>`;return`
    <article class="panel graph-panel" data-object-id="${O(e.object.object_id)}">
      <div class="panel-head">
        <div>
          <p class="eyebrow">${O(e.object.object_type)}</p>
          <h2>${O(e.object.display_name)}</h2>
          <p>${O(e.object.description)}</p>
        </div>
        <span class="badge">${O(e.canonical_state)}</span>
      </div>
      <div class="graph-summary">
        <div><span>Object ID</span><strong>${O(e.object.object_id)}</strong></div>
        <div><span>Authority</span><strong>${O(e.object.authority)}</strong></div>
        <div><span>Identity</span><strong>${O(e.object.identity_confidence_class)} · ${Math.round(e.object.identity_confidence_numeric*100)}%</strong></div>
        <div><span>Current horizon</span><strong>${O(e.horizons[0]?.name||`not assigned`)}</strong></div>
        <div><span>Judgment</span><strong>${le(e)}</strong></div>
      </div>
      <details open><summary>Identifiers (${e.identifiers.length})</summary><div class="identifier-list">${e.identifiers.map(e=>`<span><strong>${O(e.namespace)}</strong>${O(e.identifier_value)} · ${O(e.authority)}</span>`).join(``)}</div></details>
  ...
  <main class="shell">
    ...
    <section class="view" id="view-system">
      ...
      <div class="two-column">
        ...
        <article class="panel"><p class="eyebrow">Existing twin context</p><h2>Spaceport contract retained</h2><p class="muted">${fe.length} zones · ${pe.length} facility records · ${j.length} workbook tabs. The twin remains bounded context, not the command-centre homepage.</p></article>
      </div>
    </section>
...
`;var N=e=>{let t=document.getElementById(e);if(!t)throw Error(`Missing #${e}`);return t},P=N(`instruction`),F=N(`target-floor`),I=N(`priority`),L=N(`execution-mode`),R=N(`route-preview`),z=N(`command-result`),B=null,V=[],H=[],U=()=>{let e=i(P.value||`governance`);F.value=e,R.innerHTML=`<strong>Achilles route:</strong> ${e} is primary. Neo coordinates and proof returns to this gate.`};P.addEventListener(`input`,U),U();var W=()=>a({instruction:P.value,targetFloor:F.value,priority:I.value,executionMode:L.value}),G=(e,t)=>{z.dataset.tone=e,z.textContent=t},K=()=>{let e=m();N(`pending-count`).textContent=String(e.length);let t=N(`pending-commands`);if(!e.length){t.innerHTML=`<p class="muted">No locally saved commands.</p>`;return}t.innerHTML=e.map(e=>`<article class="task-card"><div><strong>${D(e.title)}</strong><span>${D(e.target_floor)} · ${D(e.priority)} · ${D(e.execution_mode)}</span><small>${D(e.command_id)}</small></div><div class="task-actions"><button data-send="${D(e.command_id)}">Send</button><button data-download="${D(e.command_id)}">Download</button></div></article>`).join(``),t.querySelectorAll(`[data-send]`).forEach(t=>t.addEventListener(`click`,async()=>{let n=e.find(e=>e.command_id===t.dataset.send);if(n)try{let e=await c(n);g(n.command_id),K(),G(`good`,`Desktop accepted ${e.command_id||n.command_id}. Task ${e.task_id??`created`}.`),await X()}catch(e){G(`bad`,e instanceof Error?e.message:`Desktop command failed.`)}})),t.querySelectorAll(`[data-download]`).forEach(t=>t.addEventListener(`click`,()=>{let n=e.find(e=>e.command_id===t.dataset.download);n&&_(n)}))},me=e=>{let t=Number(e||0);return t<1024?`${t} B`:t<1024**2?`${(t/1024).toFixed(1)} KB`:t<1024**3?`${(t/1024**2).toFixed(1)} MB`:`${(t/1024**3).toFixed(1)} GB`},q=e=>{let t=N(`desktop-projects`);if(N(`project-count`).textContent=String(e.length),!e.length){t.innerHTML=`<p class="muted">No project folders were found in the configured roots.</p>`;return}t.innerHTML=e.slice(0,30).map(e=>`<article class="task-card"><div><strong>${D(e.name)}</strong><span>${D(e.condition||`unknown`)} · ${D(e.authority||`reference`)} · ${e.file_count||0} files</span><small>${me(e.size_bytes)}${e.scan_truncated?` · bounded scan`:``}</small></div></article>`).join(``)},J=e=>{V=e;let t=N(`desktop-reviews`);if(!e.length){t.innerHTML=`<p class="muted">No project receipts are awaiting review.</p>`;return}t.innerHTML=e.slice(0,30).map(e=>{let t=e.state||`pending_review`,n=t===`pending_review`?`<div class="task-actions"><button data-review="${D(e.review_id)}" data-decision="approve">Approve</button><button data-review="${D(e.review_id)}" data-decision="hold">Hold</button><button data-review="${D(e.review_id)}" data-decision="reject">Reject</button></div>`:``;return`<article class="task-card"><div><strong>${D(e.title||`Project receipt`)}</strong><span>${D(t)} · ${D(e.event_type||`receipt`)}</span><small>${D(e.summary||e.review_id)}</small></div>${n}</article>`}).join(``),t.querySelectorAll(`[data-review]`).forEach(e=>e.addEventListener(`click`,async()=>{let t=e.dataset.review||``,n=e.dataset.decision,r=V.find(e=>e.review_id===t);if(!r||!t)return;let i=window.prompt(`${n.toUpperCase()}: ${r.title||t}\nOptional decision note:`,``)??``;try{await f(t,n,i),G(`good`,`${t} marked ${n}. Drive decision receipt written by Desktop.`),await X()}catch(e){G(`bad`,e instanceof Error?e.message:`Review decision failed.`)}}))},Y=e=>{H=e;let t=N(`representation-graphs`);t.innerHTML=de(e),t.querySelectorAll(`[data-representation-review]`).forEach(e=>{e.addEventListener(`click`,async()=>{let t=e.dataset.representationReview||``,n=e.dataset.decision,r=e.dataset.scope||`identity`,i=r===`edges`?(e.dataset.edgeIds||``).split(`|`).filter(Boolean).slice(0,100):[],a=H.find(e=>e.review?.review_id===t);if(!t||!a)return;let o=window.prompt(`${n.replace(/_/g,` `).toUpperCase()}: ${a.object.display_name}\n${r===`identity`?`Identity is reviewed before edges.`:`${i.length} bounded edges selected.`}\nOptional decision note:`,``)??``,s=window.prompt(`Enter the local owner-confirmation token. It is sent only to the loopback Desktop bridge and is not stored.`,``)??``;if(!s){G(`bad`,`Representation decision cancelled: owner confirmation is required.`);return}try{await te(t,n,r,i,o,s),G(`good`,`${t} recorded ${n}; local staging remains noncanonical until Drive readback.`),await X()}catch(e){G(`bad`,e instanceof Error?e.message:`Representation review decision failed.`)}})})};N(`command-form`).addEventListener(`submit`,async e=>{e.preventDefault();try{B=W(),G(`warn`,`Sending ${B.command_id} to Desktop…`);let e=await c(B);g(B.command_id),K(),G(`good`,`Accepted by Desktop. Task ${e.task_id??`created`}; ${e.state||`queued for governed processing`}.`),await X()}catch(e){B&&h(B),K(),G(`bad`,`${e instanceof Error?e.message:`Desktop unavailable`} The command envelope was saved locally.`)}}),N(`save-command`).addEventListener(`click`,()=>{try{B=W(),h(B),K(),G(`good`,`${B.command_id} saved locally.`)}catch(e){G(`bad`,e instanceof Error?e.message:`Command could not be saved.`)}}),N(`copy-command`).addEventListener(`click`,async()=>{try{B=W(),await navigator.clipboard.writeText(JSON.stringify(B,null,2)),G(`good`,`${B.command_id} copied as JSON.`)}catch(e){G(`bad`,e instanceof Error?e.message:`Command could not be copied.`)}});var X=async()=>{let e=N(`desktop-pill`),t=N(`desktop-state`),n=N(`merovingian-state`),r=N(`desktop-pill-text`),i=N(`desktop-tasks`);e.dataset.state=`checking`,r.textContent=`checking local runtime`,t.textContent=`Checking`,n.textContent=`Checking`;try{let a=await s();e.dataset.state=a.ok?`online`:`degraded`,r.textContent=a.ok?`local runtime connected`:`runtime connected; health needs review`,t.textContent=`Online`,n.textContent=a.merovingian?.status===`pass`?`Healthy`:`Degraded`;try{let e=await l();i.innerHTML=e.length?e.map(e=>`<article class="task-card"><div><strong>${D(String(e.title||`Untitled task`))}</strong><span>${D(String(e.status||`unknown`))} · ${D(String(e.priority||`normal`))}</span><small>Task ${D(String(e.id||``))}</small></div></article>`).join(``):`<p class="muted">Desktop queue is clear.</p>`}catch{i.innerHTML=`<p class="muted">Desktop is online, but task listing is unavailable.</p>`}try{q((await u()).projects)}catch{q([])}try{J(await d())}catch{J([])}try{Y(await ee())}catch{Y([])}}catch{e.dataset.state=`offline`,r.textContent=`start LightSpeed Desktop and the local bridge`,t.textContent=`Offline`,n.textContent=`Offline`,N(`project-count`).textContent=`0`,i.innerHTML=`<p class="muted">Desktop is offline. Commands can still be saved, copied or downloaded.</p>`,N(`desktop-projects`).innerHTML=`<p class="muted">Project registry unavailable while Desktop is offline.</p>`,N(`desktop-reviews`).innerHTML=`<p class="muted">Review queue unavailable while Desktop is offline.</p>`,Y([])}};N(`refresh-desktop`).addEventListener(`click`,()=>void X()),K(),X();var he=N(`neo-exchange`),ge=new URL(`./data/neo_exchange.json`,document.baseURI).toString();ae(async()=>{let e=await fetch(ge,{cache:`no-store`});if(!e.ok)throw Error(`Neo exchange returned HTTP ${e.status}`);return e.json()}).then(e=>{he.innerHTML=se(e)}),document.querySelectorAll(`.tab`).forEach(e=>e.addEventListener(`click`,()=>{document.querySelectorAll(`.tab`).forEach(e=>e.classList.remove(`active`)),document.querySelectorAll(`.view`).forEach(e=>e.classList.remove(`active`)),e.classList.add(`active`),N(`view-${e.dataset.view}`).classList.add(`active`)}));var _e=new URL(`./data/site_integration.json`,document.baseURI).toString(),ve=async()=>{try{let e=await fetch(_e,{cache:`no-store`});return e.ok?await e.json():null}catch{return null}},Z=()=>{if(document.getElementById(`lsgo-sites-side-edit-styles`))return;let e=document.createElement(`style`);e.id=`lsgo-sites-side-edit-styles`,e.textContent=`
