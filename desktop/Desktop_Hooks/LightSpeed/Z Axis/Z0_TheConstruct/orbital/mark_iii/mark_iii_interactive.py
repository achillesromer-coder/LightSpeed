#!/usr/bin/env python3
"""Build the Mark III v0.3.3 original-layout interactive review surface."""
...
def build_html(
    ...
    template = r'''<!doctype html><html><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>Mark III v0.3.3 Original-Layout Review</title>
...
<body><div id="app"><header><b>Mark III v0.3.3 — Original Layout / Print &amp; Plating Development</b><span class="tag" id="nodeTag">__NODECOUNT__ named nodes</span><span class="tag">25-socket dual spiral</span><span class="tag">18 body solenoids</span><span class="tag">4 propulsion interfaces · reservoir allocation</span><span class="tag">Manufacturing reference · deployment blocked</span></header>
...
function apply(){if(!root)return;const arr=arraySelect.value;root.traverse(o=>{if(!o.isMesh)return;const r=o.userData.record;if(!r)return;let v=enabled.get(group(r))&&resonatorVisible(r)&&manufacturingVisible(r);if(r.family==='array_guide')v=v&&(arr==='all'||r.array_configuration===arr);o.visible=v;const env=['service_clearance','removal_path','connector_access','cable_bend_space','array_guide','field_guide','resonator_guide','plating_allowance','layout_guide','fluid_route_reservation'].includes(r.family);o.material.transparent=env&&envelopes.checked;o.material.opacity=env&&envelopes.checked?.14:1;o.material.depthWrite=!(env&&envelopes.checked)}}arraySelect.onchange=apply;resMode.onchange=apply;mfgMode.onchange=apply;envelopes.onchange=apply;wireframe.onchange=e=>root&&root.traverse(o=>{if(o.isMesh)o.material.wireframe=e.target.checked});
