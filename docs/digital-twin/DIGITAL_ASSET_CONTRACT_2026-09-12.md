...
The Atrium review bundle contains one checked-in OBJ viewer asset and one asset metadata object per showcased twin. `data/digital-twin/generated_mesh_manifest_v1.json` binds every OBJ to its deterministic SHA-256, byte size, topology counts, units and representation class. A reproducible six-view PNG sheet (top, bottom, left, right, top-left and top-right) accompanies each OBJ and is bound by `data/digital-twin/generated_preview_manifest_v1.json`. All sixteen twins also have deterministic T1-00..T1-07 SVG review packages bound by `data/digital-twin/generated_type1_svg_manifest_v1.json`. WatchTower and M1 retain source-derived review status, Spaceport remains partial/provisional, and the remaining thirteen are visibly E-class `INTERACTION_PROXY_REVIEW_SHELL` packages with no source-binding or dimensional authority. Gated fields stay explicitly unknown. Rebuild all derivatives with the pinned generators under `tools/digital-twin/`. Two currently have stronger source authority:
